...
- **per-outcome integration KNOBS** — `intake` decides the TYPE at runtime, so one `--merge`/`--propose` can't express a type-conditional policy. Granular `--merge-spec`/`--propose-spec`/`--merge-slice`/`--propose-slice` + aggregates `--merge`/`--propose`; granular-overrides-aggregate; same-type-both = usage error; unset ⇒ propose. `intake` owns the KNOBS; WHICH knobs CI sets is CI's POLICY (`runner-in-ci`). → `intake-per-outcome-integration-modes`.

## Scope: the engine ONLY (the boundary that makes this sliceable)

**THIS SPEC's slices build the ENGINE** — all `sliceAfter: [auto-slice]`-eligible NOW, no `runner-in-ci` dependency:
...
3. the **decision prompt** (`{ask,slice,spec,bounce}` + drafted content) — a prompt asset, like the build/slicer/review prompts;
...
6. the **"is this SPEC complete?" core query** (read-only: ≥1 slice with `prd:<slug>` AND all in `work/done/`) — pure `work/`-folder logic the CI close-job consumes (verified 2026-06-06 it does not exist yet; re-check `work/done/` at slice time).

**NOT built here — these are `runner-in-ci`'s slices** (the CI scheduler/policy SPEC): trigger policy (`command`/`every-issue`, maintainer/anyone); the author-trust resolver; the autonomous **merge-vs-propose POLICY** (which per-outcome flags to pass, derived from gate state); `install-ci` + the label-driven schedule + per-issue concurrency group; the **merge-to-main close JOB** (calls the "SPEC complete?" query + `closeIssue`). `intake` is **gate-free** — an explicit invocation is its own authorization (exactly as the `do`/explicit path is not gated by `autoSlice`/`autoBuild`; slice `explicit-do-prd-not-gated-by-autoslice`); the per-repo config gates apply only to the AUTONOMOUS/auto-pick path, which is CI.

## Loop closure (the linkage this engine emits; the JOB is CI's — except BOUNCE)

For the SLICE / SPEC path the engine EMITS the linkage as a FIELD; CI ACTS on it (the issue is closed by a future CI close-job over folder + field state — `intake` never closes on the slice/SPEC path). For the BOUNCE path, by contrast, intake CLOSES the issue DIRECTLY and ATOMICALLY (terminal: comment + `reason: not planned` + close in one `closeIssue` call — a reversal recorded 2026-06-10, slice `intake-closes-issue-on-bounce`). ASK never closes. Closure is provider-agnostic by construction: the slice/SPEC link is a `work/`-file field, not a GitHub-native PR/commit keyword; the bounce close maps `reason` provider-neutrally (a non-GitHub adapter maps/ignores it).
...
- a **SPEC** fans out to N slices = N PRs → those slices carry **`prd:`** ONLY (NO `Fixes #N` and NO `Refs #N` — nothing emits a `Refs #N` keyword; `renderPrd` emits only `issue: N`); the issue is closed by CI's merge-to-main close-job running the core "SPEC complete?" query + `closeIssue`. Slices resolve the issue via `slice → prd: → SPEC issue:` (the number lives ONLY on the SPEC, never duplicated across the N fanned slices).
- **INVARIANT — one closure path per slice:** a slice uses `issue:` XOR `prd:`. A lone slice closes its own issue via `issue:`; a fanned slice contributes to a SPEC that closes the issue via `prd: → SPEC issue:`. This is documented on the `issue` field (no throwing validator); the precedence (`prd:` wins) is the close-job's concern.
...
3. As a maintainer, I want the **decision** (ask / slice / SPEC / bounce) made by a PROMPT-driven verdict the runner dispatches on (per the decision table), so the judgement is tunable like every other dorfl prompt and the dispatch is deterministic + testable.
4. As a maintainer, I want a clear small ask → ONE slice and a clear big ask (>1 slice) → a SPEC, with a coupled-but-small pair getting a light SPEC (NEVER a bounce), so the artifact matches the work and small coupled work is not wrongly rejected.
...
8. As a maintainer, I want the emitted artifact to carry its issue link as a `work/`-file FIELD (`issue: N` on a lone slice; `issue: N` on a SPEC) + its own gate axes, so it hands cleanly to the slice/build engine and to CI's closure regardless of provider (no `Fixes #N` keyword dependency).
...
The load-bearing rule: **test the DISPATCH, not the model.** A STUBBED verdict drives the dispatcher with no model/network; the prompt's JUDGEMENT is never unit-tested (like the review prompt's is not). Everything else is pure logic over stubbed inputs. STUB the seam + `gh` throughout (no network, no real GitHub). The concrete cases live in the slices' acceptance criteria — the four-outcome dispatch table (`intake-tracer-slice-outcome` + `intake-decision-prompt-and-four-outcome-dispatch`), the per-outcome mode resolution table (`intake-per-outcome-integration-modes`), the lock back-off/degrade (`intake-processing-lock`), the event-classification table (`intake-event-classification`), and the "SPEC complete?" fixture-tree cases (`prd-complete-query`).

## Autonomy notes (the gate axes)

- **`humanOnly: true` (SPEC-level, DECIDED):** reads (possibly untrusted) issues, posts under the project identity, can make work CLAIMABLE — a human should drive the SLICING of this SPEC. Per-slice (WORK-CONTRACT §3b): the dispatcher, the mode resolution, the "SPEC complete?" query, and event-classification are agent-buildable; the issue-seam ADAPTER (shells out under repo identity) leans `humanOnly`; the decision PROMPT is a prose asset a human likely tunes.
- **`needsAnswers`: NONE open.** Author-trust + the merge-vs-propose POLICY moved to `runner-in-ci` (this command is gate-free), so no open question blocks the engine slices. The decision table, the four outcomes, the lock, the per-outcome KNOBS, slug/`issue:` linkage, and the "SPEC complete?" query are all decided.

## Out of Scope

- **Everything CI** — trigger policy, author-trust, the merge-vs-propose POLICY, `install-ci`/the schedule/concurrency group, the merge-to-main close JOB (the SLICE/SPEC close, via the `issue:` field) — is `runner-in-ci`'s (see "Scope: the engine only"). `intake` is gate-free and exposes only the per-outcome mode KNOBS CI sets. NOTE: `closeIssue` itself is NO LONGER exclusively CI's — intake calls it DIRECTLY on a BOUNCE (the terminal close, slice `intake-closes-issue-on-bounce`); only the slice/SPEC close-JOB is CI's.
- **Auto-slicing the emitted SPEC / building the emitted slice** — the existing `do prd:` / `do <slice>` engine, triggered separately.
- **A throwing frontmatter VALIDATOR for the `issue:`/`prd:` invariant** — out of scope (there is no frontmatter-validation layer; intake never emits both fields, so only a human hand-edit could, and the precedence rule — `prd:` wins — degrades that to "use the SPEC's number" rather than crashing). The lone-slice `issue:` field itself IS in scope (it is the provider-agnostic lone-slice closure link, mutually exclusive with `prd:`; see Loop closure).
...
- **Absorbs the former `work/spec/issue-to-spec.md`** (deleted 2026-06-09). History: two PRDs once split this — `issue-to-spec` (every issue → a SPEC) + `issue-intake` (a slices-first front-door on top). They collapsed into one once the capability became ONE command whose conversation ends in EITHER artifact: SPEC and slice are two branches of one advance/ask loop sharing one seam / lock / trigger. The SPEC conversation, the issue seam, slug + `issue: N` linkage, and option-(iii) loop closure came from `issue-to-spec` and live here.
- **Reuse, don't reinvent:** `to-slices`/`to-spec` for the slice/SPEC shapes (and their "clear?"/"one slice?" criteria the decision prompt anchors to); `performIntegration` (`src/integration-core.ts`) for output; the review-gate's prompt→verdict→dispatch pattern for the engine shape; the slug-namespace resolver pattern for the verb. Genuinely-new: the issue SEAM (read + postComment + label ops), the decision PROMPT + dispatcher, the per-outcome mode resolution, and the "SPEC complete?" query.
- **whitesmith** (`~/dev/github/wighawag/whitesmith`) is the reference for the issue provider/seam, author-association checks, the slash-command/event workflow, and the PROVEN label + per-issue-concurrency pattern. Reuse the SEAM + concurrency pattern; do NOT reuse its label STATE-MACHINE or 1-PR-per-issue model (dorfl is 1-PR-per-slice + folder-native closure + a single transient lock label).
