...
- **C — no observation→SPEC route.** The disposition vocabulary is `promote-task | promote-adr | keep | delete | dropped | needs-attention` — there is NO `promote-spec`, and `promoteObservation` is hardwired to `tasks-ready`. So a SPEC-sized observation (a multi-task initiative needing a spec) cannot be converted in-loop. This is asymmetric with `intake`, which DOES decide task-vs-SPEC at runtime and writes `specs/<proposed|ready>/` through the shared placement resolver.
...
From the operator's perspective: when I answer an observation's question (promote or drop), the `advance` loop discharges it CLEANLY — the note leaves the inbox by deletion, and what it spawned (a task, a SPEC, or nothing) fully carries its signal. The inbox returns to being an accurate live-signal list. And when a signal is genuinely SPEC-sized, I can answer `promote-spec` and get a real spec in `specs/proposed/` instead of being forced to cram it into a task or hand-author a SPEC outside the loop.
...
4. As an operator with a SPEC-sized signal, I want a `promote-spec` disposition that mints `specs/proposed/<slug>.md`, so a multi-task initiative is converted into a spec in-loop rather than a single oversized task.
5. As an operator, I want the task-vs-SPEC sizing choice to be MY judgement (offered as `promote-task` vs `promote-spec` at the surface), never an `observationTriage: auto` auto-pick, so an initiative is never silently mis-shaped by the engine.
...
9. As an operator, I want the per-item lock / CAS guarantees that protect task promotion today to equally protect SPEC promotion, so concurrent CI legs cannot double-mint or strand a lock.

### Autonomy notes (the two gate axes)

- **`humanOnly` (DECIDED):** OMITTED — this SPEC's tasking does not require a human to drive it; the four launch questions are answered (see Resolved decisions below), so the work is straightforwardly agent-taskable.
- **`needsAnswers` (DISCOVERED):** OMITTED — the four launch questions are RESOLVED (2026-06-24). The SPEC launches fully tasking-ready.

## Resolved decisions (2026-06-24)

The four launch questions were answered by the maintainer:

1. **observation→SPEC route = triage-local CAS analogue, NOT intake's spec-emit band.** `promote-spec` mints `specs/proposed/<slug>.md` through the SAME `createItemThroughCas` helper `promoteObservation` already uses for tasks (one local commit through the CAS), just targeting `specs-proposed` instead of `tasks-ready`. Rationale: it stays inside the advance loop's existing create/integrate machinery (no `switchToWorkBranch`/`performIntegration` branch+PR band dragged in from intake, which is a standalone front door); it preserves the CAS-loser-backs-off-leaving-the-note-intact guarantee uniformly across task and spec promotion; and a promoted SPEC always lands in `proposed/` (staging) — the conservative default a human later promotes to `ready/`. The placement-resolver staged-vs-ready nuance is intake's concern (origin-trust etc.) and is NOT needed here. Sharing the spec-body RENDERING with intake may be extracted later, but the WRITER is the CAS one.
2. **Task-vs-SPEC sizing = HUMAN judgement only; `observationTriage: auto` NEVER picks SPEC.** `promote-spec` is offered at the surface alongside `promote-task` and chosen by the human answering the triage question. The `auto` gate already never auto-promotes (it only auto-disposes `duplicate`/`map`); that stays unchanged. We only add `promote-spec` to the set of dispositions a human may pick.
...
- Changing how observations are CAPTURED or SURFACED (the question-sidecar format, `surface-questions`/`answer-questions`) — this SPEC only changes DISCHARGE (apply/triage promotion + deletion).
...
- Any change to `intake` itself (it already does task-vs-SPEC correctly; per Resolved decision 1 this SPEC does NOT reuse intake's spec-emit band — it uses the triage-local CAS writer — and does not modify intake).

## Further Notes

- **Residual against US #1 (found 2026-06-25, post-landing).** The keystone task `promotion-self-contained-body-and-delete-on-promote-task-route` (now in `tasks/done/`) delivered self-containment of CONTENT, but the promoted task body still has NO `## Prompt` section, so a dispatched build throws "has no '## Prompt' section" and the per-item lock is left `state: stuck`. "Buildable on its own" (US #1) must include "passes `assembleWorkPrompt` without throwing". This is NOT re-tasked here (this SPEC is tasked + done); it is fixed by the interim task `work/tasks/backlog/promoted-task-emits-prompt-and-pre-claim-wellformedness-guard.md` (emit `## Prompt` + a pre-claim well-formedness guard), and durably by the new SPEC `work/specs/proposed/centralize-buildable-task-renderer-shared-by-intake-and-promotion.md` (the deferred Resolved-decision-1 "share the renderer with intake" extraction). Evidence: `work/notes/observations/advance-promotion-builds-promptless-task-that-self-claims-stuck-2026-06-25.md`.
- Origin: `work/notes/observations/advance-promote-leaves-resolved-note-in-inbox-and-mints-non-self-contained-stub-2026-06-24.md` (Defects A/B/C + the maintainer ruling + file:line evidence). That observation should be discharged (deleted) when this SPEC lands carrying its signal — which is itself an instance of the very behaviour this SPEC fixes.
- The three defects are one family: B (self-containment) is the prerequisite for A (delete-on-discharge), and C (SPEC route) folds into the same `promoteObservation` rework as B (branch on artifact type at the same point self-containment is built).
- **Task map (tasked 2026-06-24, born staged in `work/tasks/backlog/`):**
  ...
  - `surface-promote-prd-as-human-only-disposition` — US #5 (`blockedBy:` the promote-spec writer).
  - `work-contract-sanction-deletion-on-apply-discharge` — US #6 (`blockedBy: []`, docs-only).
  Chain: keystone → promote-spec-writer → surface; drop-route and the doc task run in parallel.
