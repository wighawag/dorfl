...
These are coupled: A is what sent the run down the (wrong) recover path on a dirty continue, where it hit a rebase conflict; B is why that FAILING dispatch left the advancing borrow orphaned (the release ran but did not land — NOT a skipped-by-throw release; the recover/conflict paths return cleanly and the `finally` release runs, so B's slice reproduces the real internal-failure cause first); C is why the resulting corruption is hard to clean. Each is independently a real defect, and together they turned a one-line continue into lost work + a stuck arbiter.
...
- **B — make the advancing-lock release crash-safe.** A FAILING post-lock dispatch (recover/integrate/gate) must still clear the `advancing` borrow, so a failed run never leaves an orphaned marker + a one-slug-two-folder ledger. (NB: the release is NOT skipped by a throw — verified that the recover/conflict paths RETURN `rebase-conflict` cleanly and the `finally` release DOES run; the release RAN and failed internally to land. The slice reproduces the real cause first.) → slice `advancing-lock-release-crash-safe` (introduces the `advancingMarkerPath(entry)` path seam).
...
4. As the runner, after ANY `advance` run (success or failure, including a recover/integrate that returns `rebase-conflict`), I want the `advancing` borrow CLEARED and the slug in exactly ONE lifecycle folder, so a failed run never leaves a one-slug-two-folder ledger or a stuck lock.
