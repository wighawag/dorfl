...
> **Launch snapshot, not maintained.** This SPEC is the source material for slicing (`to-slices`); once sliced, technical detail moves into the slices and durable rationale into `docs/adr/`. Expect this to be outrun by the work (that is fine).
>
> **RESHAPED 2026-06-12** to reflect the command surface as BUILT (it had drifted hard from a "CI = `run --once`" → "CI = `do`" snapshot written before the engine matured). Four shifts: (1) the CI entry point is no longer a single verb (CI invokes **`do`**, **`advance`**, **`intake`**, and **`gc`**, each for a different capability/trigger); (2) an `install-ci` deliverable already EXISTS in seed form (`docs/ci/advance-loop.yml.template` + `docs/ci/README.md`), shipped by the `advance-loop` SPEC, which explicitly DEFERS the unified `install-ci` surface to THIS SPEC, so this SPEC must ABSORB and supersede that seed, not compete with it; (3) the config surface is far bigger than the original auth-only view (gates, integration mode, review, per-outcome intake modes) and the wizard must surface it; (4) the design is re-derived from USER STORIES ("what is the ideal CI setup?") rather than from one engine verb. The whitesmith reference, the `models.json`/`auth.json` auth modes, the `--fake` snapshot testing, and the provider-pluggable seam are unchanged in spirit; they are made concrete here against whitesmith's proven `GitHubCIContext` adapter shape.
...
| B | **Auto-slice ready PRDs** | `do` (or `advance`) | cron + dispatch | `do prd:<slug>` / auto-pick slices a ready SPEC into backlog slices. Same verb as (A); `do` knows both rungs. |
...
| D | **Consider incoming issues → slice/SPEC** | `intake` | `issues` opened/edited, `issue_comment`, label | `intake <N>` turns an issue into the right artifact (slice / SPEC / ask / bounce). Per-outcome integration modes. Carries the author-trust axis (the one open question). |
| E | **Close issues when their work lands** | a CI close-job | PR merged to main | Runs the existing "SPEC complete?" query + `closeIssue` over `issue:`/`prd:` fields. The job is CI's; the query already exists (`prd-complete-query`, done). |
...
- **`install-ci` (the scaffolder)**: a per-capability, provider-pluggable wizard (plus a non-interactive config-file path) that generates: the GitHub Actions workflow(s) for the selected capabilities, a shared **composite setup action** (`.github/actions/dorfl-setup`) that installs Node + `dorfl` + the configured harness, configures git identity, and configures AI-provider auth, and sets the provider secrets. It ABSORBS the existing `docs/ci/advance-loop.yml.template` seed: the advance-loop capability is generated by emitting that template (parameterised) rather than hand-rolling a second workflow.
- **The verbs CI invokes** (all already built / building): `do` (A/B), `advance` (A/B/C, + the answer trigger), `intake` (D), the close-job query (E), `gc` sweep (F). No new engine verb is minted by this SPEC.
...
1. As the maintainer, I want **`dorfl install-ci`** to scaffold the GitHub Actions workflow(s) + a shared composite setup action for the capabilities I select, so autonomous work needs no local machine.
...
- **`install-ci` ABSORBS the existing seed.** `docs/ci/advance-loop.yml.template` + `docs/ci/README.md` (shipped by `advance-loop`, slice `advance-install-ci`) are the advance-loop capability's generated output. The `advance-loop` README explicitly defers the unified `install-ci` CLI surface to THIS SPEC; this SPEC claims it and emits that template as one capability. Do NOT fork a competing advance workflow.
- **The propose=matrix / merge=sequential discipline is reused verbatim** (from the seed template, validated by `src/advance-ci-template.ts`): `propose` ⇒ a MATRIX (one PR per item, enumerated via `dorfl scan --json`, each leg carries `--propose` so it can never merge to main); `merge` ⇒ a SINGLE SEQUENTIAL job (`-n <x> --merge`, because merge-mode items rebase-chain and parallel merge would thrash the main-CAS). ONE word `integrationMode` drives BOTH the integration flag AND the job shape, so they cannot desync. This is integration-mode behaviour, identical for `do` and `advance`.
- **CI / automation MUST use explicit slug prefixes** (`do slice:foo` / `do spec:foo`), never bare, because a bare slug can become ambiguous over time and would halt the job (ADR command-surface-and-journeys §3a).
...
| --- | --- | --- | --- |
| a **SPEC** | `autoSlice` | `--merge` safe (a human must slice it; it sits inert in `spec/`) | `--propose` (an agent will auto-slice it, so insert a human PR review now) |
...
So CI translates its gate state into the right mode and PASSES it to the command. For `intake` (whose output TYPE is decided at runtime), CI passes the PER-OUTCOME flags `intake` already exposes (`--merge-spec`/`--propose-spec`/`--merge-slice`/`--propose-slice` (granular) + `--merge`/`--propose` (aggregate), granular-overrides-aggregate, slice `intake-per-outcome-integration-modes`, done): e.g. `autoSlice` off + `autoBuild` on ⇒ `intake <N> --merge-spec --propose-slice`. For `do <slice>` / `do prd:` (type known up front) CI passes the single `--merge`/`--propose`. The command honors the flags; CI owns the derivation; the wizard writes the derivation into the generated workflow.
...
### Config & gate model in CI (CI = the engine gate family; always advance; install-ci one-time) (ADR `ci-config-policy-and-gate-family`)

CI is NOT a special policy surface. It runs the SAME engine policy (the `Config` gate family) in Actions, and gets a new gate for free the moment the engine has it. Three settled points:

**(i) CI always runs `advance`; the verb is never a user decision.** `advance` is a strict superset of `do`; with the lifecycle gates at their calm defaults it degrades to `do`'s build/slice behaviour. So the generated workflow ALWAYS invokes `advance` and what it does is tuned by config/env. "Calm build-only" is `advance` + both lifecycle gates off, NOT a different verb. (No `autoAdvance` gate either, the lifecycle decomposes fully into the gate family below.)

**(ii) The gate family (all per-repo + env + flag; the two question gates default CALM):**

- **`autoBuild`** (bool) gates auto-building an undeclared slice; off ⇒ rung does not run.
- **`autoSlice`** (bool) gates auto-slicing an undeclared SPEC; off ⇒ rung does not run.
- **`observationTriage`** (`off` / `ask` / `auto`, default `off`) governs the observation INBOX (raw signal): `off` ⇒ observations left untouched; `ask` ⇒ surface a promote/keep/delete question for each; `auto` ⇒ auto-dispose ONLY the no-question cases (duplicate ⇒ recommend delete / unambiguous map) and ask about the rest (never auto-deletes a non-duplicate / auto-promotes). REPLACES the old `autoTriage` boolean (`autoTriage:false ↦ ask`, `autoTriage:true ↦ auto`; deprecated alias kept).
- **`surfaceBlockers`** (bool, default `off`) governs DECLARED blocked work: whether a slice/SPEC carrying `needsAnswers:true` is rendered into an answerable question sidecar (`on`) or left silently blocked (`off`). A DIFFERENT job from observation triage (committed items vs the raw inbox), so an ORTHOGONAL peer, NOT a hierarchy: "groom my observation inbox but leave my blocked work alone" = `observationTriage: ask|auto` + `surfaceBlockers: off` is expressible.
- **apply a committed answer** is always allowed (no gate).
- **`needs-attention`** (a build hit a wall) is SEPARATE and ALWAYS on, not a "question", left as-is.

Both question gates default to their quiet state, so out-of-the-box CI builds/slices and reports failures but asks NOTHING until the user opts in. No global master "shut up" switch is needed (calm is already the default). These gates bite on the ADVANCE LIFECYCLE path, which now INCLUDES plain `run` (advance auto-pick / `advance -n` / plain `run` — the laptop daemon was unified onto the registry-set advance tick in slice `run-uses-advance-tick`, so `run --advance` is a deprecated no-op alias); only `do` has no triage/surface rungs, so they are no-ops there. Under the calm defaults plain `run` is behaviour-identical to the old build-only `run`.

**(iii) install-ci is ONE-TIME; all policy is env/config.** `install-ci` does two sticky things, set auth secrets + drop ONE fixed workflow shell, and is re-run only to rotate auth or upgrade the shell. EVERYTHING tunable (the gates above, `integration` propose/merge, `review`) is an `DORFL_*` env var / GitHub repo variable / committed `dorfl.json` key, changed WITHOUT re-running `install-ci`. The SAME `dorfl.json` the laptop uses applies in CI; the workflow's env block is just an optional CI-only override (`DORFL_AUTO_BUILD`, `DORFL_OBSERVATION_TRIAGE`, `DORFL_SURFACE_BLOCKERS`, …). So CI-vs-laptop divergence is the env block, never a CI-specific config field.

**Workflow shape (unchanged discipline, sharpened):** one fixed file, propose ⇒ a DYNAMIC matrix (default; `scan --json | jq` emits a deduplicated eligible list, one leg per id, each opens its own PR), merge ⇒ a SINGLE SEQUENTIAL `advance -n` (merge contends on `main`, so it must linearise). The CLAIM CAS, not the matrix, is the serialiser: a leg that loses the claim race exits clean, never double-builds, so the matrix is a safe fan-out convenience, not a correctness mechanism. Merge cannot use the matrix (parallel legs racing `main` = rebase thrash for no benefit).
...
The dorfl workflow surface is ONE workflow per ENABLED capability (build tick / advance loop / intake / close-job / gc sweep), NOT whitesmith's five bespoke label-state-machine workflows. Per-capability selection = which workflows/jobs the scaffolder emits.
...
- The build/slice/triage/intake/close/gc pipelines are ALREADY covered by the existing `do`/`advance`/`intake`/`gc` tests; this SPEC does not re-test them, only the CI packaging (the generated artifacts + the verb invocations they wire).

## Autonomy notes (the gate axes)

- **`humanOnly: true` (SPEC-level, DECIDED):** this is CI/auth/secrets plumbing that lands GitHub Actions workflows and sets repository secrets, a security-sensitive, infrastructure-shaping change a human should drive. **The SPEC-level `humanOnly` does NOT propagate to the slices** (resolved 2026-06-06): it means only that a human drove the SLICING. Each slice's own `humanOnly` is decided by `to-slices` from building that slice. (A pure deterministic generator with a `--fake`-snapshot test may well be agent-buildable; a workflow-writing / secret-touching slice will lean `humanOnly`. That is `to-slices`' call, per-slice.)
...
- **The BUILD/SLICE engine surface is LANDED.** `do <slug>` (in-place), `do` auto-pick / `-n` / multi-arg, `do prd:` (slice a SPEC), the `advance` verb + its five rung BODIES + drivers, `intake` + per-outcome integration modes, `gc` (worktree reap), and the "SPEC complete?" query are all in `work/done/`. The seed CI template + its validator are shipped. So capabilities A/B/D/E have their engine pieces; `install-ci` mostly WIRES them.
- **Capability C (lifecycle auto-pick) has a MISSING FOUNDATION + the two gates (engine work, not yet built).** A review pass (2026-06-12) found the advance AUTO-PICK selection enumerates ONLY eligible slices + sliceable PRDs, NOT observations or `needsAnswers`-blocked items, so the triage/surface/apply rungs are EXPLICIT-INVOCATION-ONLY today. An autonomous (bare / `-n` / plain `run` / CI) tick therefore CANNOT triage an observation or surface a declared blocker yet. The fix is the slice chain `advance-autopick-lifecycle-pools` (adds the lifecycle pools to selection) -> `observation-triage-tri-state-gate` + `surface-blockers-gate` (the two calm-default gates governing those pools) -> `run-uses-advance-tick`. **A `runner-in-ci` capability-C slice MUST `blockedBy` that chain** (at minimum `advance-autopick-lifecycle-pools` + whichever gates it relies on). Capabilities A/B (build/slice auto-pick) do NOT need it.
- **Other engine pieces still in flight (verify at slice time):** `reap-merged-remote-work-branches` (F, the `gc` merged-branch SWEEP sub-mode that `install-ci` must schedule) is in `work/backlog/`; the `advance --isolated` / `do --isolated` slices are in `work/backlog/` but DO NOT affect CI (CI is in-place). A `runner-in-ci` slice that wires capability F MUST `blockedBy` `reap-merged-remote-work-branches`.
- **The dependency-aware-scheduling enhancement is out of scope but relevant to the cron drain.** `do -n` / `advance -n` are snapshot-once (NOT dependency-aware): a slice `blockedBy:[A]` ineligible at scan time is not drained in the same run even if A is also selected and lands first (`work/observations/do-autopick-no-dependency-aware-scheduling.md`). A freshly-sliced SPEC's chained slices therefore need MULTIPLE cron ticks (or named-order invocation) to fully drain. The CI cron cadence absorbs this today; a dependency-aware `-n` would make a single tick drain a chained set. OUT OF SCOPE for `runner-in-ci` (it is a cross-verb engine enhancement), but the cron-drain docs should note the multi-tick behaviour so it is not mistaken for a bug.
- **Intake RESUMPTION is edit-blind + lock-lossy (a 3-part trigger/event-model gap to honour when wiring capability D's triggers).** Verified against current code: a user answering intake's clarifying question by EDITING their latest comment (rather than posting a new reply), and even a NEW comment landing mid-run, can be silently lost. There are THREE independent causes, and the intake-trigger design (which CI events map onto `IntakeEventKind`, and how intake resumes) must address ALL THREE, not just the first: (1) **event classifier** — `classifyIntakeEvent` (`src/intake-event.ts`) collapses EVERY `issue-comment-edited` to `ignore`, with no buried-vs-latest distinction, so an edit-to-answer never re-evaluates (a future `latest-comment-edited` ⇒ RE-EVALUATE kind would fix this while keeping `buried-prior-comment-edited` ⇒ IGNORE for loop-safety); (2) **edit-blind watermark** — the `seen=<ids>` resumption marker (`computeSeenDelta`, `src/intake-marker.ts` + `src/intake.ts`) is comment-ID-based, so editing an already-`seen` comment keeps its ID and the triage computes "nothing unseen" and SKIPS even if the classifier said re-evaluate; a complete fix must detect "edited since last seen" (track `updated_at` / a per-id body hash, not just the id); (3) **lock-release drops a mid-run trigger** — a trigger arriving while the `processing` lock is held returns `outcome:'locked'` and does nothing (no queue, no re-tick), while the in-flight run already SNAPSHOTTED the comments before the late change landed — a real lost-update window (true for a NEW comment too, not only an edit); a complete fix needs a re-check-before-release or a "dirty/re-run-needed" flag the lock release honours. Severity is silent lost-UPDATES, not merely an edit-framing miss. This is a DESIGN INPUT to capability D's trigger wiring here AND to the `issue-intake` event model (`work/spec-sliced/issue-intake.md`, the ASK-loop "resumes from the updated thread" + "buried prior-comment edit is IGNORED" lines); the engine fix itself is `issue-intake`'s, but `runner-in-ci` owns the CI event→`IntakeEventKind` mapping that must not assume comment IDs alone suffice. (Folded from the former `work/observations/intake-answer-by-editing-latest-comment-does-not-resume.md`, spotted via PR #54's Gate-3 review, re-verified 2026-06-12.)
- **Auto-slice-in-CI is OWNED by the verb, not by `runner-in-ci`.** A `runner-in-ci` slice does not BUILD "`do`/`advance` slices a ready SPEC"; it only INVOKES it (capability B). Consistent with the Out-of-Scope line.
- **CI DEPENDS ON the review gate, not the reverse.** The review gate is a property of `do`/`advance` (they run `verify` then the review step). CI inherits it by invoking the verb; it does not provide it. A slice that wants review-gated CI runs `blockedBy`s the review-gate slice; for a repo configured `review: on` + `integration: merge` (merge = auto-land on gate pass, post the `remove-automerge-merge-means-auto-on-gate-pass` slice), the CI tick is simply `do -n <N> --merge` and the gate rides along.
- **Issue-thread review surface (the review SPEC's insertion point E, OWNED HERE).** The `review` SPEC defines ONE review/edit mechanism plugged in at several insertion points; points A (slice-generation loop), C (Gate 2 PR review) and D (run coverage) are BUILT, but point **E — run the same review/edit loop over `intake`'s generated SPEC/slices and surface the reviewer's findings as QUESTIONS (and edits where sensible) back into the ISSUE COMMENT THREAD** — was explicitly deferred to the issue-intake/CI design (i.e. HERE), because it is an issue-front-door DELIVERY surface that REUSES the review machinery, not a gap in the review machinery itself. So a capability-D (intake) slice that wires the review loop into CI should surface its verdict into the issue thread via the provider comment seam (the SAME `postComment` surface `review-gate-pr-comment` already uses for PRs), not only into a `work/questions/` file. It REUSES the built `review` skill + the `slicer-review-edit-loop` / `intake-lone-slice-bounded-internal-review` loop; it adds no new review mechanism. `blockedBy` the review skill (built) + whichever intake-trigger slice owns the CI event mapping. (Carried 2026-06-12 from `work/spec/review.md`'s deferred follow-up list as that SPEC comes to rest in `spec-sliced/`; the review SPEC no longer owns E.)
...
- The SPEC→slices TRANSFORM (that is `auto-slice`/`do`/`advance`). CI only INVOKES it.
- The `gc` merged-branch sweep MECHANISM (that is `reap-merged-remote-work-branches`). CI only WIRES its scheduled trigger.
- The "SPEC complete?" QUERY (already done, `prd-complete-query`). CI's close-job CONSUMES it.
...
- The existing seed (`docs/ci/advance-loop.yml.template`, `docs/ci/README.md`, `src/advance-ci-template.ts`) is THIS SPEC's starting point for the advance-loop capability: absorb it, do not duplicate it.
- ADR `ci-config-policy-and-gate-family` records the decision (no `autoAdvance` gate; CI policy = the existing gate family resolved via the generated workflow's `DORFL_*` env block). See "Config & gate model in CI" above.
- Slice this SPEC with `to-slices`. A natural first slice is the provider-agnostic `install-ci` core + the GitHub adapter + auth (`--fake`-tested); then one slice per capability's workflow wiring (build tick, advance loop, intake, close-job, gc sweep), each independently selectable.
