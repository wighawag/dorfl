...
- **`autoSlice`** (bool) gates auto-slicing an undeclared PRD; off ⇒ rung does not run.
...
- **Issue-thread review surface (the review PRD's insertion point E, OWNED HERE).** The `review` PRD defines ONE review/edit mechanism plugged in at several insertion points; points A (slice-generation loop), C (Gate 2 PR review) and D (run coverage) are BUILT, but point **E — run the same review/edit loop over `intake`'s generated PRD/slices and surface the reviewer's findings as QUESTIONS (and edits where sensible) back into the ISSUE COMMENT THREAD** — was explicitly deferred to the issue-intake/CI design (i.e. HERE), because it is an issue-front-door DELIVERY surface that REUSES the review machinery, not a gap in the review machinery itself. So a capability-D (intake) slice that wires the review loop into CI should surface its verdict into the issue thread via the provider comment seam (the SAME `postComment` surface `review-gate-pr-comment` already uses for PRs), not only into a `work/questions/` file. It REUSES the built `review` skill + the `slicer-review-edit-loop` / `intake-lone-slice-bounded-internal-review` loop; it adds no new review mechanism. `blockedBy` the review skill (built) + whichever intake-trigger slice owns the CI event mapping. (Carried 2026-06-12 from `work/prd/review.md`'s deferred follow-up list as that PRD comes to rest in `prd-sliced/`; the review PRD no longer owns E.)
