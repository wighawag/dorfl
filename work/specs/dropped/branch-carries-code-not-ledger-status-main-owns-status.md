...
> **RETIRED / SUPERSEDED 2026-06-19.** This SPEC's own SUPERSESSION NOTICE said:
> "DO NOT slice this SPEC independently... if the lock SPEC is adopted, retire/fold
> this one into it." `work/spec/ledger-status-per-item-lock-refs.md` is now FULLY
...
> — exactly this SPEC's goals, achieved as a side-effect. The preserved PRINCIPLE
...
> **SUPERSESSION NOTICE (2026-06-17): this SPEC's MECHANISM is subsumed by `work/spec/ledger-status-per-item-lock-refs.md`** (ADR `docs/adr/ledger-status-on-per-item-lock-refs.md`). That SPEC moves ALL transient ledger status off `main` onto per-item lock refs, so a work branch cut from `main` inherits NO ledger status at all, which makes THIS SPEC's on-branch needs-attention-move removal AND the `drop-bookkeeping-rebase` deletion fall out for free (there is no transient status on a branch to drop). The PRINCIPLE here ("main owns ledger status; the branch carries code") is PRESERVED and generalised by the lock SPEC (main owns the DURABLE records; the lock ref owns the transient holds). DO NOT slice this SPEC independently of that decision: if the lock SPEC is adopted, retire/fold this one into it (the `→done` atomicity it keeps becomes the lock SPEC's durable-promotion move); if the lock SPEC is NOT adopted (the Set-1 fallback), this SPEC stands as-is. Reconcile at slicing time, not before.
...
11. As a maintainer, I want the in-flight `identify-bookkeeping-commits-by-trailer-not-rendered-todo-text` slice retired/superseded by this SPEC (it hardens a mechanism this SPEC deletes), with the disposition recorded so the backlog does not carry both.
12. As a maintainer, I want this SPEC's slices to compose with `onboard-and-reset-reconcile-mirror-to-arbiter` (arbiter is the source of truth for ledger state; the branch is code) rather than contradict it, since both express the same "main/arbiter owns status, branch carries code" principle.
...
- **`humanOnly: true` (DECIDED, set):** a human must drive the SLICING of this SPEC. It reshapes a load-bearing invariant (where ledger status lives), removes a module, and supersedes an already-written peer slice — the kind of change whose slicing benefits from human judgement on sequencing and on the retire-vs-supersede decision for the trailer slice. (Per the contract this does NOT propagate to the produced slices' own gates; individual slices may be fully agent-buildable once cut.)
...
- **The trailer slice is superseded.** `identify-bookkeeping-commits-by-trailer-not-rendered-todo-text` hardens the mechanism this SPEC deletes; record its disposition (retire it, or keep it ONLY as the interim CI-green fix until this SPEC's slices land, then delete). Do not leave both live.
...
- Auto-resolving genuine code conflicts. The "never auto-resolve a real content clash; abort → needs-attention" invariant is untouched; this SPEC removes a FALSE conflict (the runner's own bookkeeping), it does not soften the real-conflict guard.
...
- The immediate CI-green fix itself: `identify-bookkeeping-commits-by-trailer-not-rendered-todo-text` may land first as the interim mitigation; this SPEC's relationship to it (supersede/retire) is in scope, but re-litigating that slice's internal design is not.
...
- ADJACENT-BUT-DISTINCT (do NOT fold in when slicing): two backlog slices from a 2026-06-16 CI incident — `autonomous-path-auto-recovers-already-committed-stranded-branch` (a re-claim landing on an already-done-moved+committed-but-unmerged branch must integrate the kept commit / no-op, not crash with `nothing to complete`) and `autonomous-integration-refusal-surfaces-not-strands-in-progress` (an autonomous integration REFUSAL must surface to needs-attention, not silently strand `in-progress/`) — concern the branch state this SPEC KEEPS (the atomic `→done` move on the branch) and the recover/refuse paths, NOT the on-branch `→needs-attention` move this SPEC removes. They COMPOSE with this SPEC and are independent of it: this SPEC does not fix them, and they do not block it. Note especially that the failure-bounce slice is written in THIS SPEC's direction (it forbids `saveAgentFailure`'s on-branch `→needs-attention` move and surfaces tree-lessly), so the two reinforce rather than collide. When slicing this SPEC, treat them as already-cut neighbours, not as stories to re-derive.
