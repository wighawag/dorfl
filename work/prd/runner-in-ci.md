...
Both question gates default to their quiet state, so out-of-the-box CI builds/slices and reports failures but asks NOTHING until the user opts in. No global master "shut up" switch is needed (calm is already the default). These gates bite on the ADVANCE LIFECYCLE path (advance auto-pick / `advance -n` / `run --advance`); plain `run`/`do` have no triage/surface rungs, so they are no-ops there.
...
- **Capability C (lifecycle auto-pick) has a MISSING FOUNDATION + the two gates (engine work, not yet built).** A review pass (2026-06-12) found the advance AUTO-PICK selection enumerates ONLY eligible slices + sliceable PRDs, NOT observations or `needsAnswers`-blocked items, so the triage/surface/apply rungs are EXPLICIT-INVOCATION-ONLY today. An autonomous (bare / `-n` / `run --advance` / CI) tick therefore CANNOT triage an observation or surface a declared blocker yet. The fix is the slice chain `advance-autopick-lifecycle-pools` (adds the lifecycle pools to selection) -> `observation-triage-tri-state-gate` + `surface-blockers-gate` (the two calm-default gates governing those pools) -> `run-uses-advance-tick`. **A `runner-in-ci` capability-C slice MUST `blockedBy` that chain** (at minimum `advance-autopick-lifecycle-pools` + whichever gates it relies on). Capabilities A/B (build/slice auto-pick) do NOT need it.
