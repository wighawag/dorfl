...
17. As the slicer/build selector, I want a PRD or slice in the terminal `dropped/` folder to be OUT of the auto-slice / build pool BY RESIDENCE (exactly like `done/`), so a SUPERSEDED PRD is never auto-sliced, closing the gap where slicing eligibility (`needsAnswers !== true && humanOnly !== true && autoSlice && sliceAfter`) has NO notion of superseded/retired and would auto-slice a superseded PRD as if live.
18. As a maintainer, I want "superseded" to be a POSITION (a terminal folder a runner/human moves the item into), NOT a frontmatter flag, consistent with the position-vs-nature model (a durable runner/human decision is position; superseding is exactly that). So retiring a PRD is `git mv prd/<x> -> dropped/<x>` + a `reason:` in the body, removing it from the pool by residence.
...
- **Terminal "won't-proceed" record = a generic `dropped/` folder (working name), GENERALISING `out-of-scope/`.** A PRD or slice that will not proceed for ANY reason moves to `dropped/<slug>` with the REASON in the body (`superseded by <x>` / `out-of-scope` / `duplicate` / `abandoned`), instead of a folder-per-reason or a prose-only annotation the system ignores. It is a DURABLE record on `main` (the same category as `done`/`out-of-scope`), NOT a lock-ref transient. Residence in `dropped/` removes the item from the auto-slice / build pool BY RESIDENCE (exactly like `done/`), so the slicing-eligibility predicate needs NO new flag, a superseded PRD simply is not in `prd/`. "Superseded" is thus a POSITION (a runner/human `git mv`), consistent with position-vs-nature. NAME is a placeholder (`dropped`/`closed`/`shelved`/`archived`); my lean is `dropped/` (pairs with `done/`: an item ends in `done/` OR `dropped/`). This SUBSUMES today's `out-of-scope/` (it becomes one `reason:` value within `dropped/`). DECIDE the exact name + whether to fold `out-of-scope/` in or keep both at slice time. CLOSES the verified gap that slicing eligibility (`needsAnswers !== true && humanOnly !== true && autoSlice && sliceAfter`) has NO superseded/retired concept and would auto-slice a superseded PRD as if live (today only `humanOnly`/residence protects it).
...
- Terminal `dropped/`: a PRD moved to `dropped/` is NOT in the auto-slice pool (never auto-sliced) and a slice in `dropped/` is NOT in the build pool, by residence (same as `done/`); the reason is read from the body; an existing `out-of-scope/` record migrates cleanly if folded in.
