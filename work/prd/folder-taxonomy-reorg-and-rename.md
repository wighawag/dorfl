---
title: Regroup work/ under regime umbrellas (notes/ tasks/ briefs/) and rename slice->task, prd->brief, with the advancing lock co-located as a <slug>.lock.md companion
...
> **PARTIAL-SUPERSESSION NOTICE (2026-06-17): the CO-LOCATED `<slug>.lock.md` relocation (US #10-14 + the lock-format Implementation/Testing Decisions) is RETIRED as a concurrency fix by `work/prd/ledger-status-per-item-lock-refs.md`** (ADR `docs/adr/ledger-status-on-per-item-lock-refs.md`). That PRD moves the advancing lock (and all transient status) OFF `main`'s tree entirely onto per-item lock refs, so there is no longer ANY `main`-tree lock file to co-locate; the `<slug>.lock.md` companion solved neither false-contention nor branch-inheritance (it stayed a `main`-tree file) and is no longer the lock home. The REST of this PRD (the regime-umbrella regroup `notes/`/`tasks/`/`briefs/`, the `slice->task`/`prd->brief` rename, the two-phase `work-layout` migration) is UNAFFECTED and stands. When slicing: DROP the `<slug>.lock.md` relocation slice (US #10-14, story #13's lock-relocation coupling, the lock-format decisions); KEEP the taxonomy/rename/centralization work. The `advancing/` folder simply ceases to exist on `main` rather than being co-located. Reconcile at slicing time.
