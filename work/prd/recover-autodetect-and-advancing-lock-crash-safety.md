...
- **Coordination with the folder-taxonomy reorg (DECIDED 2026-06-16, cross-session).** A parallel design session (`work/ideas/folder-taxonomy-and-prd-edit-handshake.md`) will relocate the advancing-lock marker from the flat `work/advancing/<entry>.md` to a CO-LOCATED `<slug>.lock.md` sibling beside the locked item. AGREED division of labour so the two efforts do not fight over `advancing-lock.ts`: (1) B and C in THIS PRD stay on the FLAT `work/advancing/<entry>.md` path and ship FIRST; (2) Defect A is independent of the marker entirely (it touches `complete.ts`, not `advancing-lock.ts`) and ships standalone first regardless; (3) B/C MUST route ALL marker addressing through a single centralized helper pair — `advancingMarkerPath(entry)` (the one place the path is constructed, replacing the inline `work/advancing/${entry}.md` at `advancing-lock.ts` ~L187 acquire and ~L478 release) and `listAdvancingMarkers()` (the enumeration C's `gc --ledger`/`status` stuck-lock surfacing needs — net-new, since nothing globs the folder today); (4) KEEP `<type>-<slug>` in the lock BRANCH name (`advancing/<entry>`) — it is load-bearing for cross-type/cross-repo branch-collision avoidance even after the filename co-locates; (5) the taxonomy PRD's relocation slice will `sliceAfter` this PRD and REUSE `listAdvancingMarkers()` as the single seam to repoint, turning the relocation into a one-helper change. This is recorded reciprocally on the taxonomy side.
