...
3. As a maintainer, I want the **decision** (ask / slice / PRD / bounce) made by a PROMPT-driven verdict the runner dispatches on (per the decision table), so the judgement is tunable like every other dorfl prompt and the dispatch is deterministic + testable.
