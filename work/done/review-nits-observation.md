...
an APPROVE. Today (verified 2026-06-07, post-convergence) the runner routes BLOCKING
findings to `needs-attention/` but DROPS non-blocking ones on approve — they survive
only in the terminal/session log and evaporate (PR #14's gate raised 2 real nits
with nowhere to go; PR #17/#18's gates raised more). See
`work/findings/review-nonblocking-findings-disposition.md`.

> **DRIFT UPDATE 2026-06-07 — the target layer MOVED (verified).** This slice was
> written against `complete.ts`'s review/approve block. The run/do convergence
> (PR #17 `extract-integration-core` + #18 `run-through-integration-core`) MOVED the
> whole gate→review→done-move→`git add -A` band into
> `src/integration-core.ts` (`performIntegration`), and BOTH `do`/`complete` AND
> `run` now call it. So the write belongs in `integration-core.ts`'s approve
> fall-through, NOT `complete.ts`. This is a WIN: putting it in the shared core makes
> the slice's "works on ALL paths incl. --merge" promise STRUCTURALLY true (it now
> covers `run`/the fleet for free, which a `complete.ts`-only write would have
> missed).
...
- **The RUNNER writes it, not the review agent.** The reviewer emits a verdict and
  must not write (the review-skill contract); the runner (deterministic code in
  `integration-core.ts`'s `performIntegration` — post-convergence) already holds the
  parsed `findings[]` (`lastVerdict.findings`) and is the writer (it already writes
  the block case via `applyNeedsAttentionTransition`). New behaviour: the runner
  currently writes only `needs-attention/`; this adds an `observations/` write.
...
- **Git ownership — SIMPLER than the needs-attention path (call sites VERIFIED
  2026-06-07, post-convergence):** the approve case does NOT need its own
  commit/move machinery. After an approve, `performIntegration`
  (`src/integration-core.ts`) continues to the done-move + the ONE ATOMIC
  `git add -A` commit (it stages the agent's work + the `git mv` source→done
  together — steps 2 and 3 in the core). So the observation just needs to be WRITTEN
  to `work/observations/<…>.md` on disk INSIDE the approve fall-through (where
  `lastVerdict.findings` is in scope), BEFORE the `git mv` / `git add -A`, and it is
  swept into the SAME done-commit automatically — on EVERY path (merge / propose /
  CI, `do` AND `run`), with no separate commit, no separate push, and no
  surface-on-main step. This is the in-band discipline, but it is NOT the
  `applyNeedsAttentionTransition` move pattern (that helper does its own `git mv` +
  commit + autonomous surface, which is the WRONG, heavier model here — do NOT reuse
  it for the write). The block path keeps using `applyNeedsAttentionTransition`
  unchanged; only the approve path adds this plain pre-commit file write.
...
- [ ] Git ownership: the observation is written to disk INSIDE `performIntegration`
      (`src/integration-core.ts`) approve fall-through, BEFORE the core's done-move +
      atomic `git add -A` commit, so it is swept into that SAME done-commit on every
      path (merge / propose / CI, `do` AND `run`) — NOT via
...
- [ ] Works on ALL integration paths (merge / propose / CI) AND both callers (`do`
      AND `run`) — since the write lives in the shared `integration-core.ts`. A test
      exercises the merge path (no PR) and asserts the observation lands; a `run`
      path test (or a core-level test) asserts the fleet path also writes it.
...
- None — builds on the merged review gate (#11/#12) + the run/do convergence
  (#17/#18), which produces the parsed `findings[]` (`lastVerdict.findings`) in
  `integration-core.ts`'s `performIntegration` (NOT `complete.ts` anymore — see the
  drift update). Independent of `review-gate-pr-comment` (complementary sink;
  different path-coverage). (`review` flag/key, post-PR-#13 rename — `review`, not
  `reviewPr`.)
...
> `work/findings/review-nonblocking-findings-disposition.md`.
>
> FIRST run the drift check: the review/approve band MOVED in the run/do convergence
> (#17/#18) from `complete.ts` into `src/integration-core.ts` (`performIntegration`)
> — confirm THAT is where the `review` block parses `lastVerdict` with `findings[]`
> carrying `severity: 'blocking' | 'non-blocking'` (`review-gate.ts`), that the
> APPROVE fall-through currently does nothing with non-blocking findings, that the
> done-move (`git mv`, step 2) + atomic `git add -A` commit (step 3) follow it in the
> core, and that the BLOCK path uses `applyNeedsAttentionTransition`
> (`ledger-write.ts`). Write the observation INSIDE the approve fall-through (where
> `lastVerdict.findings` is in scope), BEFORE the core's `git mv`/`git add -A`, so
> the existing atomic done-commit sweeps it in — do NOT use
> `applyNeedsAttentionTransition`'s heavier move/commit/surface for the write.
> Confirm the recent `work/observations/*.md` YAML frontmatter shape
> (`title`/`date`/`status: open`) to match. Route to needs-attention on any real
> discrepancy.
>
...
>
> READ FIRST: `src/integration-core.ts` (`performIntegration` — the `review`/approve
> block, the done-move step 2 + `git add -A` step 3 it sweeps into; THIS is where the
> write goes, shared by `do` AND `run`); `src/review-gate.ts` (`ReviewVerdict`/
> `ReviewFinding`, `severity`); `src/ledger-write.ts` (`applyNeedsAttentionTransition`
> — the BLOCK-path precedent, NOT the model for the approve write); an existing
