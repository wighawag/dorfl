...
- [ ] `autoSlice` resolves flag > env (`AGENT_RUNNER_AUTO_SLICE`) > per-repo > global > default `false`, like `allowAgents` (with typed coercion + loud rejection of an invalid value, per the env layer's conventions).
