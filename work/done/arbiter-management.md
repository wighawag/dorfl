...
Commands to provision and inspect a **local `--bare` arbiter** — the offline source of truth the claim/integration protocols serialize on. Per ADR §7 (`docs/adr/execution-substrate-decisions.md`), an arbiter is precious DATA: it must NOT live under `~/.agent-runner/` and defaults under `~/git/`.

End-to-end:

- `agent-runner arbiter init [<repo>] [--at <path>] [--remote <name>]` — derive a bare arbiter from an existing working repo: `git clone --bare <repo> <arbiter-path>`, then wire the repo's `<remote>` (default e.g. `arbiter` or `origin` per config/CLAIM-PROTOCOL) to point at it. Default arbiter path is **hierarchical** under `~/git/`: `~/git/<host>/<org>/<name>.git` (config `arbitersDir`; `.`→`-` per segment, reusing the workspace encoding). `<repo>` defaults to the current repo.
- `agent-runner arbiter status` — report, for the current repo: which remote is the arbiter, its resolved path/URL, whether it exists and is bare, and whether `main` is reachable. Read-only.
...
- [ ] Default location is under `~/git/` (config `arbitersDir`), hierarchical, `.`→`-` per segment — NEVER under `~/.agent-runner/`.
- [ ] `arbiter status` reports remote name, path/URL, bare-ness, and main reachability; read-only.
- [ ] After `arbiter init`, a subsequent `claim` (via claim.sh or `agent-runner claim`) against that arbiter succeeds end-to-end.
...
> Implement `agent-runner arbiter init` and `agent-runner arbiter status` in `packages/agent-runner/`. READ FIRST: ADR §7 in `docs/adr/execution-substrate-decisions.md` (arbiters are precious data, default under `~/git/<host>/<org>/<name>.git`, NEVER under `~/.agent-runner/`), and `skills/to-slices/CLAIM-PROTOCOL.md` (the offline-arbiter setup it documents — bare by construction; a non-bare repo with main checked out rejects claim pushes).
>
