...
- [ ] `test/review-gate.test.ts` is added to the `RACE_SENSITIVE` list in `packages/agent-runner/vitest.config.ts` (it now runs in the `fileParallelism: false` project, off the parallel-load critical path).
...
>
> WHERE TO LOOK: `packages/agent-runner/vitest.config.ts` — the `RACE_SENSITIVE` array and its doc-comment (it already serialises the git-`file://`-CAS-race files like `claim-cas.test.ts` / `slicing-lock.test.ts` / `slicing.test.ts` / `slicing-integration.test.ts` into a `fileParallelism: false` project). Add `'test/review-gate.test.ts'` to that array.
>
