...
Make the **model** a first-class, resolvable, seam-passed field so agent-runner (not the operator's hand-edited command) controls which model a job runs on — across BOTH harness adapters. Auth/keys stay entirely the harness's job (ADR §13).
...
NOT in scope: per-ROLE models (build/slice/review) — staged per future capability (ADR §13); auth/keys (harness's job, never agent-runner's); install-ci wiring.

## Acceptance criteria

- [ ] `config.model` resolves flag (`--model`) > per-repo `.agent-runner.json` > global > default (unset); `model` + `harness` are honoured per-repo, `piBin` is rejected per-repo (reported), `agentCmd` stays rejected.
...
- `config-env-layer` — lands the `AGENT_RUNNER_*` env layer FIRST, so `model` (and the host-only `piBin`/`agentCmd`) get env support uniformly through the shared resolution chain, rather than this slice special-casing env for `model`. (`harness-pi`'s selector + pi adapter + `extraArgs`, and the per-repo config chain, already exist on `main`.)

## Prompt

> Make `model` a first-class field through the harness seam in `agent-runner`, so the runner controls which model a job uses across both adapters; auth/keys stay the harness's job. READ FIRST: `docs/adr/execution-substrate-decisions.md` §13 (the model/auth boundary + staging) and §5 (harness seam); and the existing code: `src/config.ts` (`Config`, `HarnessAdapter`, `agentCmd`, `harness`, `piBin`), `src/pi-harness.ts` (the pi adapter + `extraArgs` + `createHarness`), `src/harness.ts` (`LaunchInput`, NullHarness), `src/run.ts` (where it builds the harness + launches), and `src/repo-config.ts` (`REPO_ALLOWED_KEYS` / `REPO_REJECTED_KEYS`).
>
