...
RECURRED AGAIN 2026-06-10 while building `explicit-do-prd-not-gated-by-autoslice`: the unformatted base-branch file is now `work/observations/review-nits-intake-posts-completion-comment-on-slice-prd-outcomes-2026-06-10.md` (RED on `format:check` independent of this slice). Same disposition by precedent — `pnpm format` rewrapped it (pure whitespace, no content change) to keep the gate green. The cross-touch is unavoidable while the base carries unformatted observation files; the proposed one-off `format the base observations` chore is now overdue.
