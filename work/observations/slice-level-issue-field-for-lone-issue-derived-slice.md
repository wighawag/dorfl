...
- `work/backlog/intake-self-awareness-resumption-tracking.md` (a SEPARATE, pre-existing gap surfaced 2026-06-10 while reviewing the completion-comment slice: intake has NO marker / bot-identity / cursor, and `classifyIntakeEvent` re-evaluates EVERY new comment with no self-filter — so intake's OWN ask/bounce comments can re-trigger intake. Fix = a marker on intake's comments + an author/marker self-filter + feeding "my prior turns" to the decision prompt for idempotent resumption).
