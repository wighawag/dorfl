...
Because these are genuine exported CODE SYMBOLS (not `work/prds/` path literals, not the `prd:` FIELD token, not stripped-comment domain-prose — the three categorical exemptions), the forward scan MUST flag them, and they are NOT the small allow-listable set the contract task anticipates (provenance slugs / English / the 2 intake `to-prd` prompt strings). Renaming them here would silently duplicate batch 4a's atomic-rename job for symbols it deliberately curated out, i.e. an untasked source change. Routing per the task's own instruction ("if any batch is missing, the forward scan will rightly fail; route the missing batch, not weaken the scan").
