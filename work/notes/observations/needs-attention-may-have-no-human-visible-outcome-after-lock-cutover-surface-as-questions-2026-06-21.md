...
So the human-visible-outcome loss is CONFIRMED BY THE CODE'S OWN COMMENTS: stuck is NOT surfaced on `main`; the only surface is `agent-runner status`/`scan` reading lock refs (a command you must run).
