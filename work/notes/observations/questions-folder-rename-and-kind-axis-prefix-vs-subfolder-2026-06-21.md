...
- The KIND is therefore ALREADY encoded \u2014 as the FILENAME PREFIX `<type>-` (`brief-`, `observation-`, `slice-`, `spec-`, ...). On disk today `work/questions/` is a FLAT mix: `brief-land-time-...md`, `observation-...md`, etc.
...
The REAL (narrower) hazard is a SILENT-LOOKUP bug, and it gives the decision rule: subfolders are SAFE iff the subfolder is a PURE FUNCTION OF THE ITEM IDENTITY (so `sidecarPathFor(identity)` still finds the file knowing ONLY the identity, as all call sites do today). It breaks ONLY if the subfolder keys on QUESTION-KIND and one item can have DIFFERENT kinds over its life: today's `<type>` is the item NAMESPACE (`slice`/`spec`/`observation`, stable per item), but the generalized KINDS (merge/stuck/triage/spec) are a different axis. One `slice:foo` could have a SPEC question at build time and a MERGE question later at land time; if `questions/<kind>/` keys on kind, an identity-only call site computes `questions/spec/slice-foo.md` and SILENTLY misses a file now at `questions/merge/slice-foo.md` (or two sidecars in two subfolders). Silent, arguably worse than a loud git conflict.
...
CORRECTION to the two "Triaged" sections above: the promoted carrier task `task:questions-folder-rename-and-kind-axis-prefix-vs-subfolder-2026-06-21` was DELETED in commit `d4fd53db` ("repair 12 promptless promoted tasks", GROUP A) as an un-buildable promptless stub, on the plan that the fixed promotion path would re-mint it from this observation. The re-mint never happened, so the task no longer exists in any `work/tasks/*` bucket. However, its question sidecar (`work/questions/task-questions-folder-rename-...md`, 7 questions) had ALREADY been answered by a human AFTER the task was deleted (`793d3972 answer(triage)`), and those answers lived nowhere else. They are recovered verbatim below before that orphaned sidecar is removed.

These answers are the ADR-level decisions this observation asked for. They should be ratified as an ADR under `docs/adr/` (or drive a freshly re-minted task) rather than lost.

- **Q1 (Decision B, the kind axis):** Keep FLAT and identity-keyed; express kind as a TYPED FIELD in the identity HTML comment (matching how `type` is already carried), and render per-kind queues via `status`/`scan`. This is the round-2 ergonomics-only safe default and preserves the load-bearing invariant that `sidecarPathFor(identity)` is a pure function of item identity (no mutable axis in the path). Do NOT promote kind to a subfolder (that would encode a mutable axis in the path).
- **Q2 (Decision A, the rename):** Separate from B (do not conflate). Decline/defer the folder rename for now: keep the name `questions/`. B is the load-bearing structural call; A is a naming preference (inbox/attention/pending) that can be a cheap follow-up ADR + `git mv` later, with precedent in `rename-task-pool-folder-todo-to-ready`. Not worth the churn as part of this work.
- **Q3 (ADR vs slice):** Record chosen B as an ADR in `docs/adr/`, then scope the code change (add the `kind` field + per-kind rendering) as the acceptance. Folder structure/status is load-bearing, so the decision must be captured before code lands. Deliverable = both (ADR doc + code change with tests). The keying question (Q6) must be resolved in the SAME ADR.
- **Q4 (self-heal vs halt):** Out of scope here (this is about folder/kind layout). Split the self-heal-vs-halt question (should land/gc actively reconcile stale+orphan sidecars vs relying on the downstream invariant-violation HALT) into its own separate item.
- **Q5 (force-resolve should delete sidecar):** Out of scope for the layout question. Track the force-resolve-should-delete-sidecar orphan-prevention question as a separate item (it pairs naturally with Q4's self-heal item).
- **Q6 (sidecar keying):** Decide keying TOGETHER with B at the ADR level. Preserve the invariant that sidecar authorship stays on `main`/runner under the `advancing` lock (never on a work branch). Round-2 constraint stands: if merge-questions are ever authored against a branch/lock-ref instead of `main`, it re-opens the both-sides-changed rebase conflict, so the ADR must hold the line that authorship stays on `main` even when the sidecar KEYS to a branch/lock-ref identity. This is the shared decision with the needs-attention surface-state brief (fold its keying question here too).
- **Q7 (definition of done):** Do not build yet. Once A/B/keying are answered (all folded into one ADR), write a self-contained task with a `## Prompt`, explicit acceptance (the ADR written + the `kind` field + per-kind rendering code with tests + `questions/` kept as-is), and any relevant blockedBy/cross-link references.

NOTE (process gap this exposed): GROUP-A-style task deletion in `d4fd53db` removed the task file but NOT its question sidecar, orphaning it. This is exactly the orphan hazard Q4/Q5 above predicted. Captured as its own observation.
