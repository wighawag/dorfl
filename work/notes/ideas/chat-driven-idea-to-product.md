...
> This is a **pre-PRD idea**, not a committed north-star. Captured 2026-06-07 from a design conversation so it is not lost. It is NOT ready to slice: no PRD describes it, and the maintainer explicitly said "not interested to build it right now" — the value of the conversation was probing **agent-runner's capabilities** and **what (if anything) the engine itself would need**.
>
> Lifecycle tier: **`work/ideas/` (incubating) → `work/prd/` (committed north-star) → `work/backlog/` (slices) → `work/done/`.** Nothing traces to an idea; slices trace to PRDs.

## The rough thought

A web app shaped as a **chat box + a side preview**: a user types a first message (an idea), and the system advances it from idea → PRD → slices → built product, with the human's ONLY job being to **answer the questions agent-runner surfaces**, rendered as chat messages. The "side preview" was a for-instance (it could be a diff view, a file tree, or — for a web product target — the actually-built artifact served live); a web app is one interesting target among several.
...
- **`work/ideas/web-dashboard.md` — the READ-ONLY view it extends into CONTROL.** The dashboard idea is a read-only view over `scan`/`status`/`needs-attention`, and it EXPLICITLY DEFERRED a "control surface (claim/complete/retry/resolve from the browser) — mutates git / runs agents from web requests — much bigger, security- laden." THIS idea is exactly that deferred control surface, plus the conversational idea→PRD bootstrap on the front. So promoting this consciously revisits the "no web UI / HTTP control surface" boundary the original PRD locked out (same caveat the dashboard idea records).

## What agent-runner already gives this for free

- **State-in-git ⇒ a nearly stateless backend.** Each user's project is a repo with a `work/` tree on an arbiter; "the product so far" IS the repo. The server runs `agent-runner` against it; there is no app-side state machine to keep in sync.
- **The lifecycle IS a question-surfacing loop.** `prd → slices → backlog → in-progress → needs-attention → done` already means "advance autonomously until judgement is needed, then stop and surface." The chat is a different render of `needs-attention` + the advance-loop sidecar.
...
- **Conclusion: server-side agent-runner, browser is a thin client.** A demo is the right scope.
...
3. **idea → PRD → slices automation.** `to-prd` / `to-slices` are SKILLS a human invokes in a session, not `agent-runner` subcommands. The very first hop (chat message → PRD) needs an automated agent launch. The `auto-slice` / advance-loop work already moves slicing into the runner (`do prd:<slug>`); the idea→PRD hop is the remaining bootstrap. The app's server CAN launch an agent with `to-prd` directly via the harness, but it is outside the current command surface.
4. **Multi-USER orchestration + isolation.** `run` does cross-repo FLEET execution (close to what a server wants), and worktree isolation (ADR §2/§3) helps within a project — but isolating one USER's runs from another's (resource limits, fair scheduling, no starvation) is the APP's job, not agent-runner's.

## Where each piece belongs (scope fence)

- **agent-runner (the engine):** structured/JSON event output (#1). Everything else it needs for THIS app is the advance-loop work, captured separately.
- **The advance loop (its own idea/PRD):** the typed question/answer protocol (#2) and the autonomous idea-pool draining. This app is a FRONTEND of it, not a reason to build a second question mechanism.
- **The app layer (beside agent-runner, not inside it):** the chat UI, the side preview / live artifact serving, idea→PRD launch glue, and multi-user orchestration (#3 glue + #4). Sits BESIDE the runner.
...
- **Does it reverse a locked scope decision?** Like the dashboard idea, the original PRD scoped OUT a web UI / HTTP control surface. This is a CONTROL surface (runs agents / mutates git from web requests) — the heavier, security-laden half the dashboard idea deferred. Promotion is a conscious revisit of that boundary.
- **Demo vs product.** Maintainer framing was "a demo is fine." A demo (single user, one project, server-side, structured events rendered to chat) is a tractable PRD; a multi-user product (#4) is much larger. Lean: demo first.
...
Maintainer call: not the right time — the conversation's goal was to probe agent-runner's fit and gaps, not to build. It also depends on the advance-loop engine (its natural substrate) and on structured event output (the one engine change it implies). When it ripens, sequence it AFTER advance-loop and promote to `work/prd/<slug>.md` (a thin north-star), consciously revisiting the "no web control surface" boundary, then slice against that.
