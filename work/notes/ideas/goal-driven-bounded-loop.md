...
agent-runner already has the CONCEPT (an agent deliberately stopping → `agent-stopped`), but it is an out-of-band outcome inferred by the runner. Maestro's twist is an **in-band, persisted, re-run-blocking** marker the agent WRITES.

This fits agent-runner's core invariant unusually well: **"the runner owns all git-state transitions; the build agent only writes code."** A halt marker is the agent *writing a file*, not performing a git transition — the runner READS it and owns the resulting route (`git mv` to `needs-attention/` with a reason). It is a clean signalling CHANNEL from agent to runner that respects the boundary, rather than letting the agent touch lifecycle state directly. The "stale marker blocks re-runs" property maps onto the existing needs-attention hygiene (a stuck item stays stuck until a human resolves it).
...
| Maestro concept | agent-runner equivalent / required change |
| --- | --- |
...
- **Spec-first is a deliberate stance, not an accident.** A slice is a contract precisely so the runner can verify against it. A goal has no pre-written acceptance contract beyond a fuzzy exit-criteria hint — which Maestro itself admits "guides the agent; it is not matched automatically." That is exactly the thing agent-runner refuses to trust (it has a deterministic `verify` gate for a reason). A goal-loop's "done" is a self-report, not a gate. That tension is the crux.
...
- **It may already be covered.** The combination of a `needsAnswers` slice + the advance-loop's surface-question rung arguably handles "open-ended, can't fully specify" by SURFACING the unknowns as questions rather than spinning an autonomous loop. If the answer to open-endedness is "ask the human," agent-runner may not want an autonomous goal-loop at all.
...
- **Does agent-runner want open-ended goal items AT ALL,** or only the bounding mechanisms (stall guard + halt marker) applied to existing spec-first loops? (Strong lean: extract the mechanisms, NOT the goal-item, unless a concrete need for goal-items appears.)
