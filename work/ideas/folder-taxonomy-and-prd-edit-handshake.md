...
Incubates. Becomes a PRD only if the maintainer decides to reorg (then: name the folders, write the migration slice riding the swappable-paths constant, update skills / CLAIM-PROTOCOL / WORK-CONTRACT / ADR path references). The cheap wins (README-per-folder, the clarification sentence, swappable paths) can be picked up independently without committing to the reorg.
