...
**Seam change (REQUIRED, additive):** `IssueComment` (`issue-provider.ts`) today carries only `author?` + `body` (verified 2026-06-10). Add **`id`** — it is in `gh issue view --json comments` already; `normaliseComments` must surface it; the stub seam seeds it. Additive, so no existing reader breaks. (Ordering is taken from `listComments`' documented oldest-first order — NO `createdAt` is added; the triage is id-set membership + list position only, nothing reads a timestamp.)

**Precise `seen=` semantics (PIN THIS — the trap is intake's OWN comments).** `seen=` on a marker is the **per-run DELTA of HUMAN comment ids**: the ids of the comments that were in the thread when intake READ, EXCLUDING (a) intake's own marker-comments and (b) any human id already recorded in a prior intake marker's `seen=`. So `seen=` never lists an intake comment id, and never re-lists an already-seen human id. The full `seenSet` = the UNION of every intake marker's `seen=` in the thread (the CHAIN model — each marker stays bounded by per-run NEW human comments; the union reconstructs the complete set of human comments intake has ever read). Why exclude intake's own comments: the UNSEEN-CHECK below is "is there a HUMAN comment intake never read?" — if intake's own marker-comments counted, every run would see them as "unseen" and falsely PROCEED. Intake comments are identified by carrying a marker; the check ranges over NON-marker (human) comments only.

Marker shape (DECIDED): a hidden HTML comment whose namespace is built from `brand.base` (today `dorfl`, exactly like `PROCESSING_LOCK_LABEL` = `${brand.base}:processing`), so a rebrand updates it automatically:
...
1. **Last comment is INTAKE's** (the last comment carries a marker). Build `seenSet` = the UNION of every intake marker's `seen=` id-list in the thread, then:
   - **PRIMARY CHECK — is there an UNSEEN HUMAN comment?** Range over the HUMAN comments only (every comment that does NOT carry an intake marker — intake's own marker-comments are EXCLUDED, per the `seen=` semantics above); if ANY such human comment has an id NOT in `seenSet` (it landed after intake read but before it posted) → **do NOT skip → PROCEED**. The raced comment(s) are given to the prompt WITH their ordering known: they PRE-DATE intake's last turn (possibly concurrent), so treat them as possibly-already-addressed context for a PRIOR state — NOT necessarily a direct answer to intake's latest `ask`. (Position note: the raced comment sits BEFORE intake's last marker in `listComments`' oldest-first order; the check is id-membership, not position — but the ordering is what tells the prompt it pre-dates intake's turn.)
     - **DELETION ENRICHMENT (only on this proceed path):** ALSO compute `seenSet − currentHumanThreadIds`; if non-empty, a HUMAN comment intake PREVIOUSLY SAW has been deleted → tell the prompt "N previously-seen comment(s) were deleted; do not assume your prior reasoning's premises still hold" (a FLAG + COUNT — the deleted bodies are gone and not recoverable; do not try to name them). This is NOT a standalone wake trigger — it is computed ONLY because we are already proceeding for an unseen comment.
   - **otherwise** (every HUMAN comment is in `seenSet`) → **SKIP** with outcome **`no-new-input`**. Intake has the last word and saw everything up to it. We do NOT hunt for deletions here — a deletion with no NEW comment is not a turn worth waking for (it resolves naturally whenever the user next comments). This makes intake SELF-TRIGGERING a no-op BY CONSTRUCTION (intake's own freshly-posted comment carries a marker, so it is excluded from the human-comment check).
...
- [ ] `IssueComment` gains `id` and `normaliseComments` surfaces it from `gh issue view --json comments` (additive — no existing reader breaks); the stub seam seeds ids. A test pins ids are parsed. (NO `createdAt` — ordering is `listComments`' oldest-first; the triage is id-set + list-position only.)
- [ ] Every comment intake posts (ask, bounce) carries the intake MARKER (hidden HTML comment, namespace `${brand.base}:intake`) recording its `kind` (`ask` / `bounced`) AND `seen=<id>,<id>,…` = the per-run delta of HUMAN comment ids it read (EXCLUDING intake's own marker-comments AND ids already in a prior marker's `seen=`), asserted at the stubbed seam. The marker stores `kind` + `seen` only — NOT a `terminal` flag.
...
- [ ] The UNSEEN-CHECK ranges over HUMAN comments only (comments WITHOUT an intake marker); intake's own marker-comments are excluded. A test pins that intake's own marker-comments NEVER count as unseen (otherwise every run would falsely PROCEED).
- [ ] TRIAGE — last comment is intake's AND every HUMAN comment is in `seenSet` (nothing unseen) → SKIP, outcome `no-new-input` (exit 0); the prompt does NOT run; nothing emitted/posted; NO deletion hunt. Test pins it (incl. the self-trigger case: intake's own just-posted comment).
- [ ] RACE — last comment is intake's BUT a HUMAN comment has an id NOT in `seenSet` (read-then-someone-commented-then-posted) → PROCEED; the prompt receives the raced comment(s) flagged as PRE-DATING intake's last turn (context for a prior state, not a fresh answer). Test pins it.
- [ ] DELETION (enrichment, only on the race/proceed path) — when proceeding for an unseen comment, if `seenSet − currentHumanThreadIds` is non-empty (a previously-seen HUMAN comment was deleted) the prompt is told "N previously-seen comment(s) were deleted; reassess" (flag + count, no naming gone bodies). A deletion ALONE (no unseen comment) does NOT wake intake (still `no-new-input`). Test pins both: deletion+unseen → proceed-with-flag; deletion-only → skip.
...
- **Marker = hidden HTML comment, namespace from `brand.base`.** `<!-- ${brand.base}:intake kind=ask|bounced|created [slug=<slug>] seen=<id>,<id>,… -->` (today `dorfl:intake`), built from `brand.base` exactly like `PROCESSING_LOCK_LABEL`, so a rebrand updates it. Hidden (invisible in rendered GitHub), parseable from raw markdown.
- **`kind` vocabulary = `ask` / `bounced` / `created` (complete).** The marker records the kind as a NEUTRAL FACT; the TRIAGE owns whether a kind is terminal (`ask` non-terminal; `bounced`/`created` terminal). No `terminal` field in the marker — re-classifying a kind later is a triage change, old markers stay valid. (`bounced` is terminal here — the SKIP side; its terminal ACTION, closing the issue, is the sibling slice `intake-closes-issue-on-bounce`. Same "bounce is terminal" decision, two halves: this slice never re-triggers on a bounced issue; that slice closes it.)
- **The marker carries `seen=<id>,…` (HUMAN comment IDS, per-run delta) to close BOTH the raced-comment race AND deletion detection.** Ids, not a count, because a count cannot distinguish "a new comment appeared" from "an old one was deleted"; ids do both via set arithmetic. Requires an ADDITIVE seam change: `IssueComment` gains `id` (NO `createdAt` — the triage is id-set + `listComments` order, nothing reads a timestamp), surfaced by `normaliseComments`.
- **CHAIN model + HUMAN-only scope:** each marker stores ONLY the per-run delta of HUMAN comment ids (excluding intake's own marker-comments and already-seen ids); `seenSet` = union of all intake markers' lists. The unseen-check ranges over HUMAN comments only — intake's own marker-comments are identified by their marker and excluded, so they can never be mistaken for an unseen comment (the trap that would make every run falsely PROCEED).
...
>
> WHAT TO BUILD: (0) ADDITIVE seam change — `IssueComment` gains `id` (NO `createdAt`), surfaced by `normaliseComments` from `gh issue view --json comments`; stub seeds ids; (1) stamp the MARKER (recording `kind` + `seen=<id>,…` = the per-run delta of HUMAN comment ids read this run, EXCLUDING intake's own marker-comments and already-seen ids, namespace from `brand.base`) on every comment intake posts; (2) a deterministic TRIAGE under the lock BEFORE `decideAndDispatch` — build `seenSet` = UNION of all intake markers' `seen=` lists, then: last comment is intake's → PRIMARY: any HUMAN comment (one WITHOUT an intake marker — intake's own marker-comments are EXCLUDED) whose id is NOT in `seenSet` (raced-in) → PROCEED (feed it flagged as PRE-DATING; AND if `seenSet − currentHumanThreadIds` non-empty, flag "N previously-seen comments deleted; reassess"), else SKIP `no-new-input` (no deletion hunt); last comment is a human + a terminal marker (`bounced`/`created`) → SKIP `already-terminal`; else PROCEED; (3) the two new named outcomes (`no-new-input`, `already-terminal`) on `IntakeRunOutcome` + CLI exit-0 mapping. Resolve terminal-ness in the TRIAGE, never in the marker. Deletion is ENRICHMENT on the proceed path only, never a wake trigger. Do NOT touch `classifyIntakeEvent` — the triage gate is the complete guard.
>
