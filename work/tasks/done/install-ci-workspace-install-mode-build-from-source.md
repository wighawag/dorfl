...
The composite setup action emitted by `install-ci` (`generateSetupAction` in `src/install-ci-core.ts`, written to `.github/actions/dorfl-setup/action.yml`) installs the CLI with `npm install -g dorfl`. In the **dorfl repo itself** this FAILS at runtime: dorfl is not published to that npm name, so every CI job that calls the CLI dies with `dorfl: command not found` / exit code 127 (observed on the first push of the close-job workflow). The repo is self-hosting: it is both the AUTHOR and a USER of its own CI, so it must build the CLI from its own checked-out source rather than pull it from a registry.

Add a **workspace install mode** (the dorfl analogue of whitesmith's `dev` mode, see `~/dev/github/wighawag/whitesmith/src/providers/github-ci.ts`) to `install-ci`:
...
- `generateSetupAction(config)` branches on it. `registry` keeps the current `npm install -g dorfl` + harness install (UNCHANGED — the default for every consumer repo). `workspace` instead emits the build-from-source steps:
...
```
  ...
  > MONOREPO NOTE (differs from whitesmith, which is a single package): dorfl is a pnpm workspace. The build is `pnpm -r build` (not `pnpm run build`) and the link must target the CLI package specifically — `pnpm --filter dorfl link --global` — because the root package is `dorfl-monorepo` and only `packages/dorfl` exposes the `dorfl` bin (`packages/dorfl/package.json` → `"bin": {"dorfl": "dist/cli.js"}`). Verify the exact filter/link incantation builds and links a working `dorfl` on PATH before pinning it.

- **Auto-detection** (mirrors whitesmith ~L992-1003): in the orchestrator `installCI` (`src/install-ci.ts`), if `installSource` was not explicitly provided (flag/config), read `<options.ctx.workDir>/package.json` and select `workspace` when it is the dorfl monorepo, then fold that into the resolved config BEFORE calling `generateSetupAction`. Whitesmith keys off `pkg.name === 'whitesmith'`; here the root package is `dorfl-monorepo`, so detect that name (a single, exact-name check — do NOT fuzzy-match "dorfl" or it would wrongly trip in a consumer repo that happens to be named similarly). Wrap the read in try/catch and treat a missing/unparseable `package.json` as `registry` (whitesmith swallows the error). Log a one-line notice when auto-detected (e.g. `📦 Detected dorfl monorepo — using workspace install mode (build from source)`), matching whitesmith's UX.
...
This is the config-knob feature only. The human will RE-RUN `dorfl install-ci` in this repo afterwards to regenerate `.github/actions/dorfl-setup/action.yml` with the workspace steps — do NOT hand-edit the committed action.yml as part of this slice (it is generated; re-running install-ci is the supported path, per the "DO NOT hand-edit a copy — re-run install-ci" banners in the emitted files).
...
- [ ] `generateSetupAction` with `installSource: 'registry'` is BYTE-IDENTICAL to today's output (`npm install -g dorfl` + `npm install -g @mariozechner/pi-coding-agent`). The existing `install-ci.test.ts` assertions for the registry path still pass unchanged.
- [ ] `generateSetupAction` with `installSource: 'workspace'` emits NO `npm install -g dorfl`; instead emits `pnpm/action-setup@v4`, the `pnpm setup` + `$GITHUB_PATH` step, `pnpm install`, `pnpm -r build`, `pnpm --filter dorfl link --global`, and the harness via `pnpm add -g @mariozechner/pi-coding-agent`. A test pins each of these strings.
...
>
> SEAM TO TEST AT: `generateSetupAction(config)` for both `installSource` values (string assertions: registry path byte-identical to today incl. the `npm install -g` harness step; workspace path emits `pnpm/action-setup@v4`, `pnpm setup`+`$GITHUB_PATH`, `pnpm install`, `pnpm -r build`, `pnpm --filter dorfl link --global`, `pnpm add -g @mariozechner/pi-coding-agent`, and NO `npm install -g dorfl`; auth/identity/setup-node steps identical in both). Auto-detection in `installCI` with fixture repos (package.json named `dorfl-monorepo` ⇒ workspace; named otherwise / missing / unparseable ⇒ registry; explicit flag/config overrides BOTH directions). EXPLICIT `installSource` round-trips through `--export-config` → `--config`; AUTO-DETECTED `workspace` is NOT baked into the export (export from the monorepo with no explicit flag ⇒ `registry`/omitted). No network, no real `gh` (stub the provider seam as the existing tests do).
>
> DONE: registry is the unchanged default for consumer repos, workspace builds-from-source for the monorepo (auto-detected, flag-overridable), the existing registry-path tests still pass, and `pnpm -r build && pnpm -r test && pnpm format:check` passes. Do NOT hand-edit the committed `.github/actions/dorfl-setup/action.yml` (it is generated; the human will re-run `install-ci`). Do NOT perform git transitions (no stage/commit/push, no folder moves) — the runner/human owns those.
