...
Stop pinning pi's session log INTO the working directory. The pi adapter currently always passes `--session-dir <cwd>/.dorfl-pi-session`, which causes THREE bugs, all one root cause:

1. **Dashboard blindness** — sessions land outside pi's managed sessions root (`~/.pi/agent/sessions/`), so the pi-remote dashboard's `SessionManager.listAll()` never sees dorfl-driven agents.
...
3. **In-place checkout pollution** — `.dorfl-pi-session/` is written into the real checkout, is not gitignored, shows as untracked in `git status`, and `complete`'s `git add -A` can sweep it into a commit.
...
- **Config:** add a new key `sessionsDir` (the ROOT folder under which session files are generated). It is a **HOST-ONLY machine path** — the same class as `piBin` / `workspacesDir` / `humanWorktreesDir`, NOT a repo property (a committed repo file must not redirect where the host writes session logs). So:
  - resolution is **flag (`--sessions-dir`) > env (`DORFL_SESSIONS_DIR`) > global > default** — there is NO per-repo layer (do not put it in `REPO_ALLOWED_KEYS`; ADD it to `REPO_REJECTED_KEYS` in `src/repo-config.ts` so a committed `dorfl.json` setting it is ignored-and-reported, exactly like `piBin`).
  ...
  - **Compiler-forced:** `KEY_COERCIONS` in `src/env-config.ts` is an EXHAUSTIVE mapped type (`{[K in keyof Config]-?: Coercion}`), so adding `sessionsDir` to `Config` will NOT COMPILE until you add `sessionsDir: 'string'` there. This also auto-creates the `DORFL_SESSIONS_DIR` env var (the mechanical camelCase→SCREAMING_SNAKE naming).
...
- **Dashboard visibility of a fleet folder is pi-remote's job, NOT dorfl's.** pi's `SessionManager.listAll(customDir)` already accepts an explicit dir, so pi-remote can be configured to WATCH an arbitrary fleet folder (tracked in pi-remote's `work/ideas/watch-dorfl-fleet-sessions.md`). dorfl's job ends at: write `--session <path>` under the resolved `sessionsDir` and record it for liveness. Where a dashboard looks is out of scope here.
...
- Runner-driven sessions now PERSIST after a worktree is reaped (DESIRED — the audit trail survives a failed job; pi's own session retention governs cleanup, not dorfl). Just NOTE this in the slice's done record; no `gc.ts` code change is expected. (If the build finds `gc`/`reapJob` DOES reference the old `.dorfl-pi-session` path, that is drift — surface it.)
...
- [ ] A new HOST-ONLY `sessionsDir` config key resolves flag > env > global > default (NO per-repo layer; it is in `REPO_REJECTED_KEYS`, so a committed `dorfl.json` setting it is ignored-and-reported). The default is pi's default sessions location for the job cwd (a subdir under `~/.pi/agent/     sessions/`). `DORFL_SESSIONS_DIR` and `--sessions-dir` both work.
...
- [ ] In-place `do` writes NOTHING into the checkout (`git status` stays clean after a run); no `.dorfl-pi-session/` directory is created in the cwd.
...
dorfl claim session-path-pi-default --arbiter <remote>      # default --arbiter origin
