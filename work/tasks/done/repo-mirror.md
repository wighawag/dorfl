...
- **Repo→key encoding** (ADR §2): deterministic function from an arbiter remote URL to a hierarchical hub key — drop scheme/user/`.git`, replace `.`→`-` per segment (lossless), e.g. `git@github.com:wighawag/agent-runner.git` → `github-com/wighawag/agent-runner`.
- **Ensure-mirror**: given a remote URL (or name resolved from a repo), locate the bare hub mirror at `~/.agent-runner/repos/<key>.git` (config `workspacesDir`); if absent, create it (`git clone --bare` / mirror); if present, `git fetch` it. Idempotent.
...
- Treated as STATE not cache (ADR §3): lives under `~/.agent-runner/`, never `~/.cache`; regenerable but not housed where cleaners would purge it.
...
- [ ] Ensure-mirror creates a bare mirror under `~/.agent-runner/repos/<key>.git` when absent, and fetches when present; idempotent.
