...
A **per-repo config layer**: a committed config file at a repo's root (e.g. `dorfl.json`) that overrides the global `~/.config/dorfl/config.json` for that repo. This generalizes the config-resolution so repo-local properties — how this repo integrates, its `verify` gate, its arbiter — travel WITH the repo and are agreed by all collaborators and agents, rather than living only in one person's global config.

End-to-end:

- **A repo-level config file** (`dorfl.json` at repo root) holding a subset of config keys that are genuinely repo properties: at least `integration`, `verify`, `defaultArbiter` (extensible). It is committed (travels with the repo).
...
- **Multi-repo aware**: when the runner scans/operates across many repos, each repo's effective config is resolved against ITS OWN `dorfl.json` — so repo A can be `merge` while repo B is `propose` in the same run.
...
- [ ] A repo's `dorfl.json` (at repo root) is read and layered OVER the global config for that repo.
...
- [ ] A repo with no `dorfl.json` behaves exactly as today (global + defaults).
...
> Implement a per-repo config layer for `dorfl` in `packages/dorfl/`. Read the existing `config.ts`. Add a committed repo-root config file (`dorfl.json`) that overrides the global config FOR THAT REPO, for a subset of keys that are genuinely repo properties (at least `integration`, `verify`, `defaultArbiter`; extensible). Resolution is per-key, highest wins: flag (where a command offers one) > per-repo file > global > built-in default. Make it MULTI-REPO aware: when operating across repos, each repo resolves against its own `dorfl.json`, so different repos can have different integration modes in one run. Reject/ignore runner/host-only keys (`roots`, `maxParallel`, `humanWorktreesDir`, …) in the per-repo file with a clear message. A repo without the file must behave exactly as today.
>
