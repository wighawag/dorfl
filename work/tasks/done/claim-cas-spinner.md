...
When the standalone `dorfl claim <slug>` CLI command invokes the claim-cas flow (the git push that does the atomic CAS against the arbiter's `main`), the user currently sees nothing until it returns. The push can take a few seconds (network + arbiter round-trip), so the terminal looks frozen.
...
- While `dorfl claim <slug>` is running with `process.stdout.isTTY === true`, a single-line spinner animates on stderr next to a short label (e.g. `Claiming <slug>…`).
