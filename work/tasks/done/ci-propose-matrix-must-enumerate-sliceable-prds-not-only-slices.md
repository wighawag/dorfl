...
1. **Approach → (a): surface sliceable PRDs in `scan --json` and union them into the propose matrix as `prd:` legs.** Extend the `scan`/`cwd-section` JSON to ALSO report the sliceable-PRD pool (reuse `sliceablePrds` + the `work/prd` vs `work/spec-sliced` read; resolve `autoSlice` per-repo exactly as `scanMirrorPool` does — do NOT fork the predicate), then update the `jq` in the live template(s) to also emit `"prd:" + .slug` for sliceable PRDs. Surgical, keeps the propose-matrix shape, one leg per item. (Option (b), running `-n` auto-pick on the cron tick, was rejected: it loses the one-PR-per-item matrix parallelism.)
