...
> Derives from the `issue-intake` SPEC; it REVISES that SPEC's "Out of Scope" + "Loop closure" sections, so part of this slice is a drift-correction of the SPEC itself. Source signal: `work/observations/slice-level-issue-field-for-lone-issue-derived-slice.md`.
...
- **Intake SPEC outcome is unchanged** — the SPEC still carries `issue: N`; its fanned slices reach it via `prd: → SPEC issue:` (no `Fixes #N`, no slice `issue:` on a fanned slice).
- **Correct the `issue-intake` SPEC drift** (`work/spec-sliced/issue-intake.md`): the "Loop closure" section is false on BOTH closure claims — (i) "a lone slice's PR carries `Fixes #N` → its merge closes the issue directly" AND (ii) the fanned-slice "those PRs carry `Refs #N`" line (nothing emits `Refs #N` either; `renderPrd` emits only `issue: N`, and fanned slices are written later by `do prd:`). "Out of Scope" also rejects a slice-level `issue:`. Revise all three to the settled model (lone slice → `issue:` field; SPEC → `issue:`; closure is a CI close-job over folder+field state via `issue:` + `spec-complete.ts`; `Fixes #N`/`Refs #N` are deferred GitHub optimisations, NOT the mechanism). Since the SPEC lives in `work/spec-sliced/`, follow the contract's "small factual correction you are certain of" path — fix the SPEC text in place.

The actual issue-closing (the CI close-job that scans for open issues whose lone-`done/`-slice carries `issue: N`, OR whose SPEC carries `issue: N` with all referencing slices in `done/`) is `runner-in-ci`'s, NOT built here. The `spec-complete.ts` query is the existing half of it.

## Acceptance criteria

- [ ] A slice may carry an optional `issue: N` frontmatter field; the `frontmatter.ts` `issue` doc no longer says "SPEC-only" (it is parsed as a flat number already — only the type/doc + the closure reader change).
...
- [ ] Intake's SPEC outcome is unchanged (`issue: N` on the SPEC; fanned slices use `prd:`); existing SPEC-outcome tests still pass.
- [ ] The `issue-intake` SPEC's "Loop closure" + "Out of Scope" sections are corrected to the settled model (no false `Fixes #N` lone-slice-close claim; `issue:` field is in-scope; closure is the CI close-job; `Fixes #N` is a deferred optimisation).
...
>
> "Done" = a lone intake slice carries `issue: N` (not `Fixes #N`), the `issue:`/`prd:` XOR invariant is documented (no validator), the SPEC drift (both `Fixes #N` and `Refs #N` claims) is corrected, and `pnpm -r build && pnpm -r test && pnpm format:check` is green.
