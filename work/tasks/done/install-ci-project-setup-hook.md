...
This is the keystone of the `install-ci-project-provisioning` SPEC. It reuses the existing "interpolate an opaque YAML fragment into the composite action" pattern the auth-step / install-step / provider-`with:` block already use. NO presets ship here (raw escape hatch only — presets are explicitly deferred by the SPEC). NO dorfl-Node-version knob and NO conflict detection (those are out of scope / other tasks).
