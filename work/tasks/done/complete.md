...
- **Integration vocab (Q4):** the canonical modes are **`merge` | `propose`** (ADR §6, SPEC). The existing code (`config.ts` `IntegrationMode`, `integrate.ts`, and `run-once` call sites) still uses the OLD `'pr'` name — a pre-rename debt. **This slice renames `pr` -> `propose` across the code** (config type + default, `integrate.ts`, callers), keeping existing tests green, so `complete` maps config -> integration with one vocabulary. Keep `merge` as-is.
