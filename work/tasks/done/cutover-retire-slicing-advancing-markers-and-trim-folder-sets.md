...
(`backlog → done`, `spec → spec-sliced`, `backlog → dropped`).

- **Slicing:** remove `acquireSlicingLock`'s `git mv spec→slicing` marker CAS and the
  abort `slicing/ → spec/` bounce (`slicing-acquires-unified-lock` already added the
  unified `action: slice` lock + keeps the durable `spec → spec-sliced` success move —
...
- **Folder sets:** remove `in-progress`, `needs-attention`, `slicing`, `advancing`
  ...
  set: `backlog`/`done`/`dropped` for slices, `spec`/`spec-sliced` for PRDs. (`backlog`
  STAYS the pool — the `backlog → todo` rename is the deferred STEP-B SPEC, NOT this
...
> build-slice/slice-spec advance rungs are guarded SOLELY by the inner `do`'s
...
> advance∥slice on a build-slice/slice-spec item remain mutually exclusive through the
...
- [ ] Slicing no longer writes the `slicing/` marker (nor the abort bounce); the
      unified `action: slice` lock is the sole exclusion; the durable `spec →
      spec-sliced` success move is unchanged.
...
- [ ] `in-progress`/`needs-attention`/`slicing`/`advancing` are removed from
      ...
      set remains (`backlog`/`done`/`dropped`; `spec`/`spec-sliced`). No
...
> slicing's `git mv spec→slicing` marker + abort bounce (`slicing-lock.ts` —
> `slicing-acquires-unified-lock` already added the unified `action: slice` lock and
> keeps the durable `spec → spec-sliced` success move; only the transient marker goes),
...
> (`backlog`/`done`/`dropped`; `spec`/`spec-sliced`). Retarget any remaining
> `slicing/`/`advancing/` reader (`advance.ts`, `slicing.ts`, `review-gate.ts`,
> `cli.ts`). SPEC US #5, #7; ADR `docs/adr/ledger-status-on-per-item-lock-refs.md`.
>
> `backlog` STAYS the pool (the `backlog → todo` rename is the deferred STEP-B SPEC; do
...
> prove (test) that advance∥claim and advance∥slice on a build-slice/slice-spec item
