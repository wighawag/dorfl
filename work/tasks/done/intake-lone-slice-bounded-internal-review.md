...
- [ ] `runSliceReviewLoop` is NOT imported or called from `intake.ts`; the PRD, ASK, and BOUNCE outcomes are untouched (only `dispatchSlice` / the `slice` branch changes). Verified by inspection.
