...
So REUSE `gc.ts`'s existing §4 deletion-safety predicate, which already checks BOTH (1) working tree CLEAN (`git status` INSIDE the worktree) AND (2) branch tip REACHABLE on the arbiter. Enumerate the replaced mirror's worktrees via `discoverJobs(workspacesDir)` (walks `<workspacesDir>/work/*` for `.dorfl-job.json`, each carrying the mirror key) and/or `git worktree list`, and run the clean-AND-reachable predicate in EACH. `--force` proceeds only when EVERY worktree is provably safe; a single dirty-or-unreachable worktree blocks `--force` with a clear message.
