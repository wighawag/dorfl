...
- **GATHER-only:** formulate questions by composing `review` (for a slice / SPEC / code) + the native triage question (for an observation: promote / keep / delete)
...
- **The humility rule:** surface the residue, NEVER invent an answer. (No answer-creation — that is REJECTED by design in the SPEC.)
- **Human-invokable for the no-runner path (US #34):** a human may invoke it by hand and persist via the `advance` verb (the apply rung — a later slice; NOT `do advance`, since `advance` is a SIBLING top-level verb and `do` subcommands are REJECTED in the SPEC) or by hand-writing the documented sidecar format from `advance-sidecar-contract` — with NO separate write-skill added unless hand-writing proves annoying.
...
- [ ] It composes `review` (slice/SPEC/code) + the native observation-triage question + pre-existing `needsAnswers`/`## Open questions`; it does NOT duplicate `to-slices`/`review` (they stay the single sources, unchanged).
