...
3. **Blast radius.** Every current and future caller of the shared buildable-body renderer (task promotion path, SPEC promotion path, and the pending sibling task that rewires the intake path to the same renderer) must obey this split. A renderer caller that emits its own trailing blank after frontmatter, OR a future renderer edit that adds a leading blank at the heading, reintroduces the fence-spacing drift class.
