...
> 2. Thread `cwd` down the call chain so the resolver gets the target repo root: `renderPrompt` (`options.cwd`), `do.ts` + `run.ts`'s `buildAgentPrompt(slice.slug, slice.spec, slice.slicePrompt, {...})` call sites (they have the repo `cwd` — today they pass only `continueContext`), and `cli.ts`'s `render-prompt` action (`process.cwd()`) → `buildAgentPrompt(..., {cwd})` → `wrapper(..., {cwd})` → `resolveClaimProtocolPath(cwd, override)`. Keep the `protocolPath` override short-circuit so existing tests that inject a path are unaffected.
