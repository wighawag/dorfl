...
- [ ] `do slice:<slug>` / `do prd:<slug>` resolve via `slug-namespace-resolution`; `do prd:<slug>` reaches the slicing-path entry point (full slicing is `autoslice-command`, not built here); a bare-slug slice/PRD collision errors.
