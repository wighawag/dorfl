...
- For the `do prd:` SLICING path this works by accident: `titlePath` points at an ALREADY-EXISTING held SPEC, so the read succeeds.
...
- Do NOT regress the `do prd:` slicing path (where `titlePath` is an existing SPEC and the read already succeeds) — whatever the fix, that path must keep its current title.
...
>
> SCOPE FENCE: lone-SLICE title path only; do NOT touch the SPEC/ASK/BOUNCE outcomes or the `do prd:` title behaviour beyond a regression guard.
>
