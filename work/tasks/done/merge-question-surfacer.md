...
(`merge | hold | drop`), which the keystone SPEC
...
`decide(input, allowedOutcomes) -> {task | spec | adr | delete | ask}`
...
2. The two cross-cutting questions SHARED with the stuck-lock surfacer
   sibling, which must be answered ONCE across both (see the spec's
...
- The cross-cutting questions in the spec's "Part of a larger
  generalization" section (sidecar-keying, questions-folder shape) —
  those are see-also, not in this spec's scope.
