...
The SECOND expand step (§3a), added because the first expand task (`expand-spec-frontmatter-and-namespace-aliases`) covered the frontmatter/namespace/config/intake surface but MISSED the lock/sidecar identity — so `lockEntryFor('spec:foo')` currently falls through to `task-foo` (silent collision with task-build locks). This task ADDS a `'spec'` member to the lock/sidecar identity BESIDE `'prd'`, so a later batch can emit `spec-<slug>` locks/sidecars safely. Purely additive; removes nothing.

- **`sidecar.ts`:** add `'spec'` to `SidecarType` (`'prd' | 'task' | 'observation' | 'spec'`); add `spec: 'spec'` to `TYPE_TO_NAMESPACE`; add a `spec` case to `typeForNamespace` (a `spec:` prefix / `explicit === 'spec'` → `'spec'`, NOT the `task` fall-through). Keep the `'prd'` member and its mapping intact.
- **`item-lock.ts`:** add the `'spec'` case wherever `'prd'` is switched/listed — the `case 'prd':` around L884 (add a sibling `case 'spec':`) and the `['task', 'prd', 'observation']` prefix loop around L1545 (add `'spec'`), plus `terminalMainPaths`/`itemFromLockEntry` if they enumerate the namespace. Keep `'prd'` working.
...
- [ ] `SidecarType` includes `'spec'`; `TYPE_TO_NAMESPACE`/`typeForNamespace` map `spec` to `'spec'` (not the `task` fall-through); `item-lock.ts` handles `spec` everywhere it handles `prd`.
...
> Goal: the SECOND expand step of the identity-layer wide refactor (read `work/protocol/TASKING-PROTOCOL.md` §3a + the parent spec `work/specs/tasked/prd-to-spec-vocabulary-cutover-and-migration-command.md`). Add a `'spec'` member to the LOCK/SIDECAR identity BESIDE `'prd'`, so `spec:<slug>` gets its own `spec-<slug>` lock entry instead of silently falling through to `task-<slug>`. Purely additive — remove/migrate nothing.
>
> Why this exists: the first expand task widened frontmatter/namespace/config/intake but MISSED the sidecar/lock identity. `SidecarType` in `sidecar.ts` is still `'prd' | 'task' | 'observation'` and `typeForNamespace` maps a `spec:` prefix to the `task` fall-through, so `lockEntryFor('spec:foo') === 'task-foo'` — a collision that breaks the per-item lock's isolation invariant. The `do` agent building batch 2 caught this (see work/notes/observations/spec-lock-namespace-not-expanded-migrate-batch2-blocked-2026-07-09.md).
>
> Where to look: `sidecar.ts` (`SidecarType` L72, `TYPE_TO_NAMESPACE` L~182, `typeForNamespace` L~188), `item-lock.ts` (`case 'prd'` ~L884, the `['task','prd','observation']` loop ~L1545, `terminalMainPaths`/`itemFromLockEntry`). Add the `spec` sibling everywhere `prd` is handled. Add a `lockEntryFor('spec:x') === 'spec-x'` test.
>
