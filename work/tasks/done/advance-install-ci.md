...
- The `install-ci` notion: a one-step adoption (a command or a documented copy of the template) so a repo opts into the CI loop without hand-assembling the workflow. (Whether `install-ci` is a CLI subcommand or a documented template copy is a micro-choice — pick the lighter one and record it in `## Decisions`; the SPEC calls it "the `install-ci` notion", not a hard CLI surface.)
...
```sh
dorfl claim advance-install-ci --arbiter origin
...
- The workflow_dispatch `mode` input (propose/merge) only selects WHICH CI job runs (matrix vs single sequential); it does NOT pass any --integration flag to `advance`, so the actual propose/merge integration behaviour (open PR vs merge-to-main) is decided solely by the repo's dorfl.json default and can DESYNC from the workflow mode. Concretely: dispatch `mode: propose` (the default) with config `integration: merge` produces a parallel matrix where every leg merges to main concurrently - exactly the main-CAS thrash the slice says merge mode must avoid. Tie shape-selection and integration-mode together (pass --merge/--propose or --integration ${ADVANCE_MODE} on the advance invocations driven by the same input) and reconcile the vocabulary so 'mode' is not two different concepts sharing one word. (docs/ci/advance-loop.yml.template: matrix leg runs `dorfl advance "${{ matrix.item }}" --arbiter origin` and merge job runs `dorfl advance -n 10 --arbiter origin`, neither with an integration flag; ADVANCE_MODE / the `if:` guards only gate job selection. CONTEXT.md:38 defines 'integration mode' = propose (open PR) / merge (to main), DEFAULT_INTEGRATION='propose' (complete.ts:290, config.ts:302). Slice criterion: merge mode must be a SINGLE SEQUENTIAL job because 'parallel merge jobs would thrash the main-CAS'.)
...
Gate-2 found a REAL wiring bug (gate-1 green 1616/1616). The workflow's dispatch 'mode' input (propose/merge) only selects the JOB SHAPE (matrix vs single-sequential) via the if: guards, but the advance invocations themselves (docs/ci/advance-loop.yml.template L128 'advance ${{matrix.item}} --arbiter origin' and L142 'advance -n 10 --arbiter origin') pass NO integration flag — so the actual open-PR-vs-merge-to-main behaviour comes from the repo's dorfl.json default and can DESYNC from the workflow mode. The dangerous case: mode:propose (matrix, parallel) + config integration:merge → every parallel matrix leg merges to main concurrently = exactly the main-CAS thrash the slice says merge mode must avoid. FIX: tie shape-selection and integration-mode together — pass --merge/--propose (or --integration ${ADVANCE_MODE}) on BOTH advance invocations so the integration mode the legs use ALWAYS matches the shape the dispatch input selected (propose⇒matrix⇒--propose; merge⇒single-sequential⇒--merge). Confirm 'advance' actually accepts --merge/--propose/--integration (mirror 'do'/'complete'); if the flag name differs, use advance's real one. Reconcile the 'mode' vocabulary so the one word isn't BOTH the job-shape selector AND the integration mode (e.g. name the input integrationMode and derive the shape from it, or document that mode drives both). Add/keep a template test asserting the matrix leg carries --propose and the sequential job carries --merge (no leg can merge in a parallel matrix). Do NOT restart — CONTINUE from this branch's good template/scaffold and just thread the integration flag + reconcile the vocabulary + assert it.
