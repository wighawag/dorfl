...
The THIRD (and final) missing migrate sub-batch the C-audit dropped (see the agent STOP diagnosis captured this session). Two pieces, both ratified with the human (2026-07-10): (A) a CLEAN-BREAK rename of the intake integration-mode CLI flags, and (B) a FULL PURGE of every residual internal `prd` code identifier so the contract task's forward scan can be maximally strict (catch even private names) and dorfl's source is genuinely `prd`-free except the deliberate alias/field/folder-literal/prose.

### Piece A — intake integration-mode CLI flags: CLEAN BREAK to `spec` (no back-compat alias)

The `--merge-prd`/`--propose-prd` family was a DOCUMENTED deferral (`intake.ts:425`, observation `intake-trigger-template-prd-flag-surface-stays-prd-in-batch-3.md`) that no batch owned. Human decision: CLEAN BREAK (matching the cutover's hard-cutover spirit; NO `--merge-prd` deprecated alias).

- **CLI flag definitions** (`cli.ts:3688/3692`): `--merge-prd` → `--merge-spec`, `--propose-prd` → `--propose-spec` (+ the `--merge-prd`/`--propose-prd` help strings + the `intake` command description at `cli.ts:3664` that lists them).
...
- **CI trigger-template** (`intake-trigger-template.ts`): the emitted shell `prd_flag="--propose-prd"/"--merge-prd"` (`:345/:347`), the `prd_flag` output var (`:380/:383/:396`), and the doc-comments/policy prose (`:33/:103/:129-130/:142-145/:161/:208/:244/:330-332`) → `spec_flag` / `--propose-spec`/`--merge-spec`. Update the coupled trigger-template test assertions (`:512-519` `derives-merge-prd`/`--merge-prd\b` → `--merge-spec`).
- NOTE the compiled `.github/workflows/*.yml` are NOT edited here (a human regenerates them via `install-ci`, exactly as the `403a5be9` `brief→prd` cutover excluded compiled workflows). Flag this regeneration as a manual follow-up in the done note.

### Piece B — FULL PURGE of residual internal `prd` identifiers → `spec`

Rename every remaining `prd`-spelled CODE identifier (exported OR internal: consts, functions, methods, local vars, import aliases) to its `spec` spelling. These are file-local or small-blast so each is safe; do them so `pnpm -r build` stays green. Known set (grep to confirm + catch any this list misses — the acceptance criterion is a clean grep, not this list):
...
- **Local variables** (file-local, rename freely): `prdCandidates`→`specCandidates`, `eligiblePrds`→`eligibleSpecs`, `prds`→`specs`, `prdPool`→`specPool`, `prdContent`→`specContent`, `prdPath`→`specPath`, `prdRel`→`specRel`, `prdFile`/`prdTaskedFile`→`specFile`/`specTaskedFile`, `prdBase`→`specBase`, `prdGranular`→`specGranular`, `prdTasked`→`specTasked`, `prd` loop vars → `spec`, etc., across `advance-drivers.ts`, `do-autopick.ts`, `ledger-read.ts`, `lifecycle-gather.ts`, `mirror-pool-scan.ts`, `needs-attention.ts`, `prompt.ts`, `scan.ts`, `tasking.ts`, `tasking-lock.ts`, `intake.ts`.
