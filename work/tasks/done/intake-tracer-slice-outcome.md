...
- [ ] Gate axis note (deliberate): this slice carries `humanOnly: false` (omitted) on purpose — its build-nature is mechanical (a stubbed-verdict dispatcher + an inline prompt builder + `gh` confined to the adapter), and `intake` is gate-free. This is a deliberate per-slice decision, NOT an oversight, notwithstanding the PRD autonomy note's lean toward `humanOnly` for the seam adapter / prompt.
...
> 2. The `intake <N>` command + a decision step returning a VERDICT object (`{ask,slice,prd,bounce}` + drafted content). A thin first-draft PROMPT asset is enough here (the full asset is the next slice).
...
>
> SCOPE FENCE: build ONLY the `slice` branch of the dispatcher (ask/prd/bounce are `intake-decision-prompt-and-four-outcome-dispatch`). Do NOT build the per-outcome mode KNOBS (`intake-per-outcome-integration-modes`), the processing LOCK (`intake-processing-lock`), event-classification (`intake-event-classification`), or the "PRD complete?" query (`prd-complete-query`). Do NOT build ANY CI/policy (trigger, author-trust, merge-vs-propose POLICY, install-ci, the close job) — that is the `runner-in-ci` PRD. Do NOT add an issue-label state-machine (ADR §12) — no labels in this slice at all (the lock is a separate slice).
>
> FIRST run the drift check (launch snapshot): confirm `performIntegration` (`src/integration-core.ts`) is still the shared integrate core; confirm no `intake` command / issue seam already exists in `packages/agent-runner/src`; confirm `ReviewProvider.postComment` still exists to be renamed. If any premise drifted, route this slice to `needs-attention/` with the discrepancy (WORK-CONTRACT.md "Drift is a needs-attention signal") rather than building on a stale premise.
>
