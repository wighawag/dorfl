...
- [ ] The PRD `work/spec-sliced/review.md` is updated to Model P (review auto-lands iff `integration: merge`; `propose` = human merges) — its `autoMerge`-as-separate-knob language is removed. The ADR `docs/adr/ci-config-policy-and-gate-family.md` `autoMerge` reference is updated/removed to match.
