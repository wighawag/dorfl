...
> Harden the dorfl `work/` ledger so the integration DONE-MOVE is atomic against the ARBITER's current status folder and the one-slug-one-folder invariant holds on that transition. This closes defect 1 (orphaned `in-progress/` after merge — a "move" became a "copy", PR #86, hand-cleaned in `279b542`) and its root, defect 2 (a transition could publish a slug in two folders). Source SPEC: `work/spec-sliced/ledger-integrity.md` (read it for the full root-cause trace; it may be in `work/slicing/` until this slicing lands).
>
