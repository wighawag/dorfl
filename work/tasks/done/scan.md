...
- [ ] Per item, the AFK gate resolves correctly for afk true / false / unspecified against both `allowUnspecifiedGate` settings.
- [ ] An item is eligible only when its gate passes AND every `blocked_by` slug is present in that repo's `work/done/`.
- [ ] Output shows, per item: repo, slug, afk gate, blocked-by status, eligible?
- [ ] The deterministic core (config-merge, detection, eligibility, frontmatter parsing) is unit-tested with vitest against fixture directory trees.

## Blocked by

- `scaffold` — needs the monorepo skeleton (CLI package, tsc/vitest toolchain) in place before any command can be built.
...
>
> Test the deterministic core first (TDD, vitest in `test/`): config-merge, repo detection, frontmatter parsing, eligibility resolution — against fixture directory trees. The CLI surface uses `commander`. Match the repo house style (scaffolded from `template-typescript-lib`: NodeNext, tabs + single quotes, `type: module`). "Done" = the acceptance criteria above are met and tests pass.
