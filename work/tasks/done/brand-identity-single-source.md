...
Make the **load-bearing protocol identity** of agent-runner derive from a SINGLE source-of-truth module, so that renaming the project later changes one base string instead of risking a missed, silently-breaking occurrence (a missed env var fails silently — the worst kind). The name is very likely to change; this de-risks that.

Centralize ONLY the **protocol/contract surface** — the strings that are a contract with the user / filesystem / CI and that BREAK if inconsistent — derived from one base identity (`agent-runner`) via the standard case transforms:

- the **`AGENT_RUNNER_` env-var prefix** (constantCase of the base + `_`) — today built/spread across `env-config.ts` / `repo-config.ts` / `harness.ts`;
- the **`.agent-runner.json` per-repo config filename** (`REPO_CONFIG_FILENAME` in `repo-config.ts`);
- the **`~/.agent-runner/` workspaces dir default** — hardcoded in `workspace.ts`, `work-on.ts`, `gc.ts`;
- the **binary / package name** (`agent-runner`) where referenced from code (not the `package.json` `name`/`bin` themselves — those are renamed by the tool below, not indirected through a runtime constant).
...
- [ ] The `AGENT_RUNNER_` env prefix, `.agent-runner.json` filename, and `~/.agent-runner/` default are all DERIVED from that module (no remaining hardcoded literals of these at the derivation sites in `src/`).
...
- `config-env-layer` — it just introduced the `AGENT_RUNNER_*` env layer (`env-config.ts`) and reshaped `repo-config.ts`; this slice centralizes the prefix it established, so it must land first (also avoids editing the same files concurrently).

## Prompt

> Centralize agent-runner's **protocol/brand identity** behind a single source-of-truth module so a future rename is one change, not a scattered (and silently-breakable) find/replace. PURE REFACTOR — observable behaviour must be byte-identical (the derived strings equal today's literals).
>
> READ FIRST: `src/env-config.ts` + `src/repo-config.ts` (the `AGENT_RUNNER_*` env prefix + `REPO_CONFIG_FILENAME = '.agent-runner.json'`), `src/workspace.ts` / `src/work-on.ts` / `src/gc.ts` (the `~/.agent-runner` workspaces-dir default), `src/harness.ts` (an `AGENT_RUNNER_MODEL` mention), and `packages/agent-runner/ package.json` (`name`/`bin` = `agent-runner`). Also `skills/to-slices/WORK- CONTRACT.md` (this is a contract-sanctioned PRD-less chore slice).
>
