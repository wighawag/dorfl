...
Fix the build/slice **gate asymmetry**: `do prd:<slug>` currently refuses an EXPLICITLY-named SPEC when the repo's `autoSlice` policy is off, but `do <slice>` (build) does NOT refuse an explicitly-named slice on `allowAgents`. The contract is that `autoSlice`/`allowAgents` are the **auto-pick / pool-eligibility** policy ("may an agent slice/claim _UNDECLARED_ items here?") — they gate the `run` / `do` auto-pick / CI pool, NOT a target the operator explicitly typed. The slice path drifted by re-applying the policy on the explicit form; this slice removes that re-gate so the two paths are symmetric.

End-to-end behaviour after this slice:

- `do prd:<slug>` (an EXPLICITLY-named SPEC) **slices regardless of the `autoSlice` policy** — naming the SPEC IS the authorization, exactly as `do <slice>` builds a named slice regardless of `allowAgents`. (No more `DORFL_AUTO_SLICE=true` workaround for an explicit slice-now.)
- The **item-readiness axes still bind** on the explicit path: a `humanOnly: true` SPEC (a human must drive the slicing) and a `needsAnswers: true` SPEC (open questions) are STILL refused for the agent `doer`, and `sliceAfter` ordering is STILL enforced. Only the repo's `autoSlice` POLICY is dropped from the explicit path. (These are the SPEC's own readiness, not the repo's auto-pick policy.)
- The **auto-pick pool is UNCHANGED**: `do` (no arg) / `do -n <x>` still build the sliceable-SPEC pool through `autoslice-gate`'s predicate (which includes `autoSlice`) in `do-autopick.ts`, so an unselected/ineligible SPEC is still never auto-picked. The policy keeps doing its real job (pool eligibility) — it just stops double-gating the explicit named dispatch.
...
- [ ] `do prd:<slug>` on an explicitly-named SPEC slices with `autoSlice` OFF (no config, no env) — asserted via a test that drives the explicit path with the policy unset and confirms it proceeds to the lock/agent (not `gate-refused`).
- [ ] A `humanOnly: true` SPEC is STILL refused on the explicit agent path; a `needsAnswers: true` SPEC is STILL refused; an unsatisfied `sliceAfter` is STILL refused — only the `autoSlice` policy axis is removed from the explicit gate.
- [ ] The auto-pick SPEC pool (`do` / `do -n`) STILL filters by `autoSlice` (a SPEC ineligible by the policy is never auto-picked) — assert the pool path is unchanged (the `do-autopick`/`selectPrioritised` SPEC-pool test still gates on `autoSlice`).
...
> - `src/do.ts` (`resolved.namespace === 'spec'` → `performSlice`, do.ts ~457): the explicit dispatch. This is the call site that must NOT carry the policy gate.
> - `src/do-autopick.ts` (`performDoAuto`): the AUTO-PICK pool — it builds the sliceable-SPEC pool via `autoslice-gate`'s predicate (incl. `autoSlice`). This must stay (the pool is where the policy legitimately lives). A SPEC selected FROM the pool then dispatches into the same `performSlice`; ensure removing the per-invocation gate does NOT let a pool-ineligible SPEC through (it can't be auto-picked in the first place, but confirm the pool filter is the single enforcement point).
...
>
> SEAM TO TEST AT: the `slicing.ts` gate unit tests (assert the explicit agent path slices with `autoSlice` off, but still refuses `humanOnly`/`needsAnswers`/unsatisfied `sliceAfter`) + the `do-autopick` SPEC-pool tests (assert the pool STILL filters on `autoSlice`). Throwaway-git / stubbed-harness per the house style.
>
