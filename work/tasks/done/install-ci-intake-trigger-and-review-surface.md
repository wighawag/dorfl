...
(The two PRD design inputs are RESOLVED in the Decisions section above, so this slice is buildable. The deeper lock-release lost-update window for a NEW comment landing mid-run remains an `issue-intake` ENGINE concern — `work/spec-sliced/issue-intake.md` — not a blocker for shipping this CI wiring.)
