...
- [ ] `dorfl prompt <slug>` prints the wrapper + the slice's `## Prompt`, with `<slug>` and the source PRD path substituted.
...
> Implement `dorfl prompt [<slug>]` in `packages/dorfl/`: print to stdout the full work-agent prompt for a slice. READ FIRST: CLAIM-PROTOCOL.md ("the prompt handed to the work agent" \u2014 the canonical wrapper text), the PRD's agentCmd-prompt decision, and the existing frontmatter/markdown parsing in the scan core.
>
