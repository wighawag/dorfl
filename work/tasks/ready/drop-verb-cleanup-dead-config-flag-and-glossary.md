...
2. In `CONTEXT.md`, add a one-line glossary entry (in whatever glossary/terminology section exists, or a new short one if none) pinning:
   ...
   - **`dropped`** — the PRD won't-proceed TERMINAL (`prds/dropped/`, per WORK-CONTRACT.md) and the triage disposition value (per SURFACE-PROTOCOL.md): the file is RETAINED with the reason in the body.
