---
title: Build the self-contained dorfl prd-to-spec migration command (engine + fixture test)
...
The new `dorfl prd-to-spec` CLI verb: self-contained (decision B), mechanical DATA migration only (it does NOT author contract text — the contract arrives via the setup-re-sync it invokes). Thin command shell over a reusable data-migration ENGINE (user story 9).

Behaviour:
1. **Quiescence check (refuse loudly):** clean working tree AND no held lock AND no in-progress `work/prd-*`/`work/spec-*` work-branch carrying unlanded work. On failure, exit non-zero naming the offending lock/branch. (Matches ADR §7e decision 1a.)
...
3. **Data migration (deterministic, all four layers):** (a) FOLDERS `work/prds/{proposed,ready,tasked,dropped}/ → work/specs/…` via `git mv`; (b) FRONTMATTER/body `prd: → spec:`, `taskedAfter` prose, path refs — across ALL data items INCLUDING `work/tasks/done/` and `work/specs/tasked/` (done-items ARE converted — determinism); (c) CONFIG `.dorfl.json` (`prdsLandIn → specsLandIn`, intake type keys, `taskingIntegration` values naming prd folders); (d) LIVE GIT REFS (lock-refs `refs/dorfl/lock/prd-<slug>` and work-branches `work/prd-<slug>` — but per the quiescence gate there are none held/in-flight, so this is renaming only inert refs).
...
Test at the behavioural seam with a FIXTURE repo (temp/scratch, isolated — do NOT touch the real repo or a real home dir): a `work/` tree carrying all four layers (a `prds/*` item, a task with `prd:` frontmatter incl. a `done/` item, a `.dorfl.json` with `prdsLandIn`, an inert `prd-<slug>` ref/branch) → assert deterministic conversion, accurate `--dry-run` (no writes), idempotency, and refuse-on-dirty-tree / held-lock / in-progress-branch naming the offender, and a green leak scan on the converted fixture.

## Acceptance criteria

- [ ] `dorfl prd-to-spec` exists; runs quiescence → setup-re-sync → data migration; `--dry-run` + idempotent.
...
> Goal: build the self-contained `dorfl prd-to-spec` CLI verb — quiescence check → setup contract re-sync → deterministic four-layer DATA migration (folders, frontmatter incl. `done/`, config, inert refs) → `--dry-run` + idempotent, with the forward+reverse leak scan as the acceptance gate. Thin command shell over a REUSABLE engine. Read the parent spec `prd-to-spec-vocabulary-cutover-and-migration-command` (esp. ADR §7e, user stories 4–9) + `work/protocol/CLAIM-PROTOCOL.md` for the command conventions.
>
> Domain vocabulary: the migration splits at the contract-version boundary — the CODE/CONTRACT already speak `spec` (prior tasks); this command migrates a repo's DATA to match. Decision B = self-contained (setup-re-sync first, then data). Quiescence-required (1a) = refuse on dirty tree / held lock / in-progress `work/prd-*` branch. Done-items ARE converted (determinism). The engine is factored so a future cutover reuses it, but the verb stays purpose-named.
>
...
>
> FIRST check drift: confirm the source-part cutover fully landed (batches 1–5 + contract) — the command depends on `work-layout.ts` already saying `spec`. If any `prd` identity survives in the code, that source-part task has not landed and this one should wait.
