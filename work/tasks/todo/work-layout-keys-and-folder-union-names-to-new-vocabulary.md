...
calls `workItemRel('prd-sliced', …)` while its docstring says "a BRIEF:
`work/briefs/tasked/…`").
...
- `WORK_FOLDER_NAME` keys: `pre-backlog → tasks-backlog`, `backlog → tasks-todo`,
  `pre-prd → briefs-proposed`, `prd → briefs-ready`, `prd-sliced → briefs-tasked`.
...
- The folder-union arrays + their types that still carry slice/prd vocabulary:
  ...
  consumer (`prompt.ts`, `prd-complete.ts`, `close-job.ts`, `ledger-lint.ts`,
...
- [ ] No `'pre-backlog'` / `'pre-prd'` / `'prd-sliced'` symbolic KEY, and no
      `'prd'`/`'backlog'`-as-a-folder-key, remains in `src/` (they are renamed); the
      slice/prd-named folder unions + types are renamed to task/brief vocabulary and
      every consumer updated.
- [ ] `terminalMainPaths` (item-lock.ts) reads `workItemRel('briefs-tasked', …)`
      (not `'prd-sliced'`), matching its docstring.
...
> stale `work-layout` symbolic KEYS and the slice/prd-named folder-union
...
> still has keys `pre-backlog`/`backlog`/`pre-prd`/`prd`/`prd-sliced` mapping to the
...
> `prd-complete.ts`, `close-job.ts`, `ledger-lint.ts`, `integration-core.ts`,
