...
- If yes, it is recorded as `promptGuidance.testFirst: true` in `.agent-runner.json`.
...
1. As a maintainer, I want to declare `promptGuidance.testFirst: true` once in `.agent-runner.json`, so every AFK build in this repo is nudged to write the failing test first, without my having to repeat it per task.
...
- **New config key `promptGuidance` (object) in `.agent-runner.json`**, first member `testFirst: boolean`. NOT under the gate family; a distinct namespace whose name says "nudge". Default off (omitted = false).
...
- Test EXTERNAL behaviour at the prompt-assembly seam: given a resolved `promptGuidance.testFirst` (on/off, repo-default, item-override), assert the strengthened test-first line is present/absent in the assembled worker prompt. Prior art: `packages/agent-runner/test/prompt.test.ts` already asserts on assembled prompt text and wrapper resolution.
