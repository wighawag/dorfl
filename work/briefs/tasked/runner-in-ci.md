...
**(iii) install-ci is ONE-TIME; all policy is env/config.** `install-ci` does two sticky things, set auth secrets + drop ONE fixed workflow shell, and is re-run only to rotate auth or upgrade the shell. EVERYTHING tunable (the gates above, `integration` propose/merge, `review`) is an `AGENT_RUNNER_*` env var / GitHub repo variable / committed `.agent-runner.json` key, changed WITHOUT re-running `install-ci`. The SAME `.agent-runner.json` the laptop uses applies in CI; the workflow's env block is just an optional CI-only override (`AGENT_RUNNER_AUTO_BUILD`, `AGENT_RUNNER_OBSERVATION_TRIAGE`, `AGENT_RUNNER_SURFACE_BLOCKERS`, …). So CI-vs-laptop divergence is the env block, never a CI-specific config field.
