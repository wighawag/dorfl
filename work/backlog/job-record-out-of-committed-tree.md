...
3. **Remove the now-unnecessary gitignore + exclude hacks.** Once the record is out of the tree it can NEVER be staged, so:
   - Remove `.agent-runner-job.json` from this repo's `.gitignore` (it no longer lives in any tree); and STOP `setup`/scaffolding from adding that gitignore line to target repos (grep `skills/setup` + any scaffold for the entry and remove it — the protection is now structural, not gitignore-dependent).
   - Remove the `:(exclude)${JOB_RECORD_FILENAME}` / `.filter(… !== JOB_RECORD_FILENAME)` guards in `src/agent-stop.ts` (~L205/L260) and `src/gc.ts` (~L371) that only existed to defend against the in-tree record — VERIFY each is purely that defence before removing (some may also exclude `work/`; keep the `work/` half if so). The goal: no code path needs to special-case the record filename in a git operation anymore.
...
- [ ] The `.gitignore` entry for `.agent-runner-job.json` is REMOVED (here and from `setup`/scaffold), and the `:(exclude)`/`.filter` job-record guards in `agent-stop.ts`/`gc.ts` are removed (keeping any co-located `work/` exclusion that is a separate concern) — verified no git operation special-cases the record filename anymore.
