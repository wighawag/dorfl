...
- **Correct the `issue-intake` PRD drift** (`work/prd-sliced/issue-intake.md`): the "Loop closure" section is false on BOTH closure claims — (i) "a lone slice's PR carries `Fixes #N` → its merge closes the issue directly" AND (ii) the fanned-slice "those PRs carry `Refs #N`" line (nothing emits `Refs #N` either; `renderPrd` emits only `issue: N`, and fanned slices are written later by `do prd:`). "Out of Scope" also rejects a slice-level `issue:`. Revise all three to the settled model (lone slice → `issue:` field; PRD → `issue:`; closure is a CI close-job over folder+field state via `issue:` + `prd-complete.ts`; `Fixes #N`/`Refs #N` are deferred GitHub optimisations, NOT the mechanism). Since the PRD lives in `work/prd-sliced/`, follow the contract's "small factual correction you are certain of" path — fix the PRD text in place.
