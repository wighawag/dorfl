...
- Rename the config key `allowAgents` → `autoBuild` across the config surface: `.agent-runner.json` (the per-repo file), `config.ts` / `env-config.ts` / `repo-config.ts` (the `REPO_ALLOWED_KEYS` entry, the resolution chain, the `AGENT_RUNNER_*` env var, the CLI flag `--allow-agents`/`--no-allow-agents` → `--auto-build`/`--no-auto-build`), and the docs / WORK-CONTRACT references.
...
- After this, the gate family reads symmetrically: `autoBuild` (build a ready slice) / `autoSlice` (slice a ready PRD) / `autoTriage` (auto-disposition an observation) — all default off, all resolved through the same chain.

## Acceptance criteria

- [ ] `autoBuild` is the new name across `.agent-runner.json`, `config.ts` / `env-config.ts` / `repo-config.ts` (allowed keys, resolution chain, env var, CLI flag), docs, and WORK-CONTRACT.
...
>
> READ FIRST: `packages/agent-runner/src/repo-config.ts` (`REPO_ALLOWED_KEYS` has `allowAgents` — rename + alias), `packages/agent-runner/src/config.ts` + `env-config.ts` (the resolution chain + `AGENT_RUNNER_*` env var + the CLI flag), the `rename-reviewpr-to-review` migration (the precedent alias/deprecation pattern — look in `work/done/` / git history), and the docs / WORK-CONTRACT references to `allowAgents`. Grep the whole codebase for `allowAgents`.
>
