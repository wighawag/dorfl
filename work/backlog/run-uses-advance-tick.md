...
- [ ] The docs the original STOP flagged are reconciled: ADR `ci-config-policy-and-gate-family.md` §5 and PRD `runner-in-ci.md` (~L118) currently say plain `run` is build-only and `run --advance` is the lifecycle path (the INVERSE of "plain run becomes advance"). Update them to match the landed behaviour (plain `run` = advance with calm-default gates).
...
>
> BUILD: point plain `run`'s per-item unit (the `RunTick` the loop drives) at the precursor's registry-set advance tick. RECORD the `--advance`-alias / `--build-only` decision (default: plain `run` becomes advance, `--advance` a deprecated alias, no `--build-only` unless a real divergence surfaces; reconcile with whatever the precursor did to the single-mirror `run --advance` path). Reconcile the ADR §5 + PRD `runner-in-ci.md` L118 wording (plain `run` build-only -> plain `run` = advance).
>
...
>
> "Done" = plain `run` runs the registry-set advance tick, is behaviour-identical under calm defaults, performs the lifecycle when a gate is flipped, the borrow is race-correct, the ADR/PRD wording is reconciled, and the gate is green.
