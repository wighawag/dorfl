...
(`seen=` lists the comment ids intake READ this run — the per-run delta.) Hidden HTML comments render as NOTHING on GitHub (clean thread) but are present in the raw markdown `listComments` returns, so the triage parses them. The marker is provider-PORTABLE and is the SOLE recovery signal (no sidecar/cursor).
...
>
> SEAM TO TEST AT: the stubbed issue seam (`postIssueComment` records the marker; `listComments` seeds threads with ids, markers, raced-in and deleted comments) + the triage branches (`no-new-input` / race-proceed / deletion-enrichment / `already-terminal` / proceed) + the `seenSet` union across two markers + `classifyIntakeEvent` (own-comment → ignore). The prompt JUDGEMENT is not unit-tested (PRD); only the triage + dispatch. Mirror the existing intake + intake-event tests.
>
