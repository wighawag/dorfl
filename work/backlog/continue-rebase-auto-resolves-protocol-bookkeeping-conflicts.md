...
- **The done-move** — `… → done`. `done/` ASSERTS "this work is merged." It therefore CANNOT be a free-standing move on main; it MUST ride the SAME ref update that brings the code in — i.e. it is a commit ON the work branch (or folded into the integration commit) so that the PR merge lands `done/<slug>.md` AND the code on `main` atomically. **Keeping the done-move on the branch is REQUIRED by the invariant; do NOT move it off.**
...
WHY this is essential, not a safety net (the sharp case): the done-move (`mv in-progress→done`) MUST stay on the branch (invariant) and the integration sequence REBASES it onto the arbiter (`integration-core.ts`: `done-move → commit → rebase-onto-arbiter → verify → integrate`). If main independently moved the SAME `work/<slug>.md` via a tree-less bookkeeping move between claim and integration (a surface, a requeue), then rebasing `in-progress→done` onto a main where the file now sits in `needs-attention/`/`backlog/` produces the SAME rename conflict — with Part 1 done, this is the ONLY remaining conflict, but it is INHERENT to the done-move and Part 1 alone does NOT remove it. So Part 2 is what actually makes the done-move robust; without it the conflict simply recurs at integrate-time with one fewer move involved.
...
- **If the branch commit being replayed is the integrating DONE-MOVE** (the branch is landing its code): resolve to `done/` — the branch WINS the destination, BECAUSE the code is arriving in this same integration. This does NOT violate the invariant: `done/` appears on main precisely together with the code (the whole point of keeping the done-move atomic). Main's concurrent bookkeeping placement (needs-attention/backlog) is SUPERSEDED by the completed work. Notes UNION.
- **If the branch commit being replayed is NOT a done-move** (e.g. a leftover continue with no integration happening): the **arbiter's `main` is the source of truth** for bookkeeping placement; resolve to main's folder, notes UNION. The resolver does NOT fabricate `done/` here — only an actual integrating done-move earns `done/`.
...
- [ ] **The DONE-MOVE's own rebase auto-resolves:** when the integrating done-move (`in-progress→done`) is rebased onto a main that concurrently moved the same `work/<slug>.md` via a tree-less bookkeeping move (surface/requeue), the rebase resolves to `done/` (the integrating branch wins the destination, atomic with the code) and completes `clean` — it does NOT route to needs-attention. A test pins exactly this (claim → done-move on branch; main surfaces/requeues the same .md; integrate; assert clean + file in done/ + code present).
- [ ] A NON-integration continued-branch rebase whose ONLY conflicts are the slug's `.md` placement across BOOKKEEPING folders and/or appended notes is AUTO-RESOLVED to MAIN's folder (arbiter wins; notes union), completes `clean`, and does NOT fabricate `done/`.
- [ ] **Invariant preserved:** `done/<slug>.md` is written to main ONLY by an integrating done-move carrying the code in the same ref update — NEVER by resolving a non-integration bookkeeping conflict. A test asserts a non-integration resolve never yields `done/` without code.
