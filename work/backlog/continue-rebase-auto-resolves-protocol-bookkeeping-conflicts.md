...
- When `git rebase <main>` conflicts, inspect the conflicted paths. If EVERY conflicted path is under `work/` AND is the slug's `.md` placement across the BOOKKEEPING folders (`backlog`/`in-progress`/`needs-attention`) and/or runner-appended note bodies, RESOLVE deterministically: the **arbiter's `main` is the single source of truth for bookkeeping folder placement** (the branch's view is always stale), and appended notes UNION. Then continue the rebase.
- **The resolver MUST NEVER place the slug in `done/` as part of conflict resolution** — `done/` only ever arrives via the branch's own atomic `→done` move merging with the code. If a conflict somehow implicates `done/`, that is NOT a pure-bookkeeping conflict: `--abort` → needs-attention (do not invent a merged state). This is the invariant made executable.
- If ANY conflicted path is OUTSIDE the bookkeeping set (a real source/content conflict), keep EXISTING behaviour exactly: `--abort` + `{kind: 'conflict'}` → needs-attention (ADR §10 unchanged).

> Companion primitive: the existing tree-less move in `advance-treeless-publish.ts` is the right vehicle for the bookkeeping moves on `main`. Do NOT repurpose it for the done-move (the done-move is integration, not bookkeeping).
