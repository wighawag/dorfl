...
- [ ] DECISION (record while building): does `off` suppress ONLY the first-pass surface, or also the `apply` of an ALREADY-surfaced+answered sidecar? Default position: `off` means "do not CREATE new blocker sidecars"; an already-answered sidecar should STILL apply (never strand a human's committed answer). Implement that and note it; if a test shows otherwise is needed, record the reversal.
...
>
> BUILD: (1) the `Config` boolean field + default `false`; (2) the full chain (REPO_ALLOWED_KEYS, boolean env coercion, CLI flag); (3) the read site at the surface rung's declared-`needsAnswers` path. RECORD the two decisions in the criteria (off suppresses first-pass surface but NOT apply-of-an-answered-sidecar; explicit invocation bypasses).
>
