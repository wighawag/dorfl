...
>
> In packages/agent-runner/src/scan.ts the two propose-matrix pool gates resolve per-repo config through DIFFERENT readers within one repo iteration:
>  - autoBuild (~L368): resolveRepoConfig({repoPath: mirror.path}) — working-tree reader pointed at a BARE mirror path, which cannot read a committed .agent-runner.json and falls back to global/default.
