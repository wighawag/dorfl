...
> The observation captures two defects in `agent-runner advance`'s pool-check at `src/claim-cas.ts:270` and `:332` (both returning `{exitCode:2, outcome:'lost'}` with the literal `'<backlog>' not found on <arbiter>/main (already done/removed, or wrong slug).'`, mapped to exit 2 by `src/do.ts` ~L553):
