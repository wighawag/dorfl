...
- **NOT a prd-producer of its own.** `to-spec` writes the prd (it owns the prd shape, the `work/protocol/spec-template.md`, the launch-snapshot banner, the `prds/ready/` target, the two autonomy axes). from-idea is the FRONT DOOR that calls it. Read the two names as a pair: `from-idea` owns the on-ramp (setup + the interview); `to-spec` is the synthesis primitive it hands the conversation to. Do not reimplement any of to-spec here.
...
```
clarify the idea  →  setup (PLAN → HARD STOP → scaffold)  →  to-spec (write prds/ready/<slug>.md, unstaged)
```

`setup` MUST run first: `to-spec` writes to `work/prds/ready/`, which requires `work/` to exist. The flow has **two natural human checkpoints**, and from-idea honors both:
...
2. **The prd landing unstaged in `prds/ready/`** — `to-spec` leaves the file in the working tree for the human to review; it does not stage/commit. from-idea inherits that etiquette: **never stage/commit/push** at any step.
...
**The interview floor (the stop condition).** Clarify enough that `to-spec` can write a coherent launch snapshot. Borrow to-spec's own spine — a prd-worthy idea has:
...
- the fuller **problem + shape + seams + open questions** is what to-spec synthesizes into the prd.
...
**Git etiquette:** never stage, commit, or push — leave both setup's scaffold and to-spec's prd in the working tree for the user to inspect and commit (the producer-skill convention setup and to-spec both follow).

## Boundary (what from-idea does NOT do)

- It does NOT reimplement detection (setup A1), the gate (setup A3/A3b), `CONTEXT.md` (setup A2), the prd shape / banner / target (to-spec + `spec-template.md`). Its only net-new surface is the idea interview + ordering the two calls + feeding the description to setup.
