...
- **Don't** use it to judge an item already IN the pool (that is plain `review`), to build a ready task (`drive-tasks`), or to slice a brief (`to-task`). It is the ONE gate-crossing judgement: staging → pool.
...
3. **Pool-readiness gate** — once in the pool, can the item rest there SAFELY? Promote if yes. The bar is "safe in the pool", NOT "claimable this instant":
   ...
   - **Brief** → `humanOnly`/`needsAnswers` correct and it is genuinely sliceable (not still a design sketch). Like `blockedBy`, `briefAfter:` is ENFORCED against `briefs/tasked/` residence by the auto-slicer, so an unsatisfied `briefAfter:` is NOT a reason to keep a sliceable brief staged — promote it and let the slicer sequence it. An unready (design-sketch / question-bearing) brief stays `briefs/proposed/`.
