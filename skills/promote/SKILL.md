...
- **Use** before admitting a staged task/brief into the pool: when reviewing what an agent (tasking / intake) emitted into `tasks/backlog/` or `briefs/proposed/`, to decide promote / keep-staged / drop for each. `orchestrate` composes this in its survey (the "awaiting promotion" set); a human also reaches for it directly to clear a staging backlog.
- **Don't** use it to judge an item already IN the pool (that is plain `review`), to build a ready task (`drive-tasks`), or to task a brief (`to-task`). It is the ONE gate-crossing judgement: staging → pool.
...
3. **Pool-readiness gate** — once in the pool, can the item rest there SAFELY? Promote if yes. The bar is "safe in the pool", NOT "claimable this instant":
   ...
   - **Brief** → `humanOnly`/`needsAnswers` correct and it is genuinely taskable (not still a design sketch). Like `blockedBy`, `briefAfter:` is ENFORCED against `briefs/tasked/` residence by the auto-tasker, so an unsatisfied `briefAfter:` is NOT a reason to keep a taskable brief staged — promote it and let the tasker sequence it. An unready (design-sketch / question-bearing) brief stays `briefs/proposed/`.
...
5. **Verdict** — emit ONE of:
   ...
   - **KEEP-STAGED** — a fixable gap that is NOT machine-enforced: a drifted premise, an open question (`needsAnswers`), a wrongly-set `humanOnly`, or a brief that is still a design sketch. (An unresolved `blockedBy` / `briefAfter:` is NOT such a gap — it is enforced, so it is a PROMOTE, not a keep-staged.) State the SPECIFIC gap so it can be resolved, then re-checked.
