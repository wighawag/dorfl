---
name: setup
description: "The ONE skill to onboard ANY repo onto the file-based work/ contract (the protocol agent-runner consumes) — it detects the repo's state and does the right DEPTH automatically. On an empty/near-empty repo it just scaffolds (work/ skeleton, work/protocol/ docs copied verbatim, CONTEXT.md, a stack-appropriate .agent-runner.json verify gate) after a short adoption conversation. On a populated/legacy repo it ALSO converts existing material into the contract: a task tracker / TODOs / design docs into PRDs, slices, or ideas (a vague wish is an idea, NOT a needsAnswers slice); the codebase split by polarity (EXTERNAL behaviour it integrates with into work/findings/ with a source/provenance, our own code's shape into CONTEXT.md/docs — NOT findings); and deliberate decisions into ADRs by ELICITING the why from the human (asked live; NEVER inferred — no why means no ADR, the code stands as its own pre-ADR). Always presents an inventory + plan and STOPS for confirmation before writing the judgement-heavy parts; never clobbers; never auto-commits. Use whenever someone wants to set up / adopt / onboard / migrate a repo to the work/ contract or agent-runner — empty or full, this is the single entry point."
---
...
- **Detect Phase B material — and look in DOTFOLDERS too, not just the visible tree.** While inventorying, note any **convertible material**: a `tasks/` folder, a `TODO.md`, an issue-tracker export, `docs/` design notes / RFCs, plan files, and **substantial source code** (code that embodies decisions). Crucially, **also scan the interesting dotfolders** — plans and notes from editor/agent tooling routinely hide in hidden dirs (anything that looks like it could hold design docs / plans / tasks). Do NOT stop at a plain `ls` of the visible top level; a missed dotfolder means a missed source (e.g. a rich design doc under a tool's hidden plans dir that should become a PRD, silently under-routed to an `idea`). **Skip the noise**: `.git/`, `node_modules/` and other dependency dirs, build output, and anything `.gitignore`d — those are never sources. This detection decides whether Phase B runs (see A4 / Phase B). A repo with only a README that says "clean slate" (or nothing) → Phase B is empty; finish after Phase A.
...
- **CHECKPOINT 2 — Source-cleanup: for EVERY source you fully converted, you MUST explicitly propose deleting it (with confirmation) before finishing.** Once a source (`TODO.md`, a `tasks/` folder, a plan doc — including ones in dotfolders — a design note) is _fully_ captured into `work/` items, leaving the original creates **two sources of truth that drift**. So enumerate, in the report, each fully-converted source and ASK to delete it ("<source> is now captured in <where> — delete it?"). This is not optional and not a side note: a run that converts sources but never proposes their cleanup has left the repo with dual truth. Only delete on explicit user confirmation; never `rm` on your own initiative; never propose deleting a source you only _partially_ converted (say what's left); if the user declines, leave them and note they are superseded. Reaching the end having converted ≥1 source but proposed deleting NONE is a RED FLAG you skipped this checkpoint.
