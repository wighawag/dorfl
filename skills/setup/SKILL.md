...
**Nudge to PIN the dorfl version (reproducibility — language-agnostic; fold into the A4 plan, do NOT add a separate question round).** Agents are taught to invoke bare **`dorfl`**, so the workflow stays project-independent — but bare `dorfl` runs whatever version is globally installed, which DRIFTS (a laptop on one version, CI floating to latest, a repo reasoned-about under a third). A repo can PIN the dorfl version it runs with, so builds/advances/intakes are reproducible. ASK once, generically: **"Do you want to PIN the dorfl version this repo runs with, so `dorfl` is reproducible instead of floating with whatever is globally installed? (recommended for CI.)"** If yes, record the pin the language-appropriate way and note it under `## Conventions` in CONTEXT.md so it is discoverable: for a **JS repo**, add `dorfl` to the ROOT `package.json` `devDependencies` at the current version (the `install-ci` CI shim already PREFERS a project-local `node_modules/.bin/dorfl` over the global bootstrap, so CI then uses the pin automatically); for a **non-JS repo**, pin via whatever its environment resolves (a vendored `./bin/dorfl`, an `npx dorfl@<version>` / `mise` / `asdf` shim, etc.). Do NOT invent a JS dependency in a non-JS repo (the A3 language-agnostic rule). If the repo/dorfl gains a `dorflBin` pin field in `dorfl.json` (a bare version that bare `dorfl` self-forwards to), prefer THAT as the single declaration; until then, the environment-level pin above is the mechanism. If they skip, leave it — the global `dorfl` is used (floating); note the trade-off in one line so it is a conscious choice.
...
<!-- Reproducibility: consider PINNING the dorfl version this repo runs with, so bare `dorfl` does not float with whatever is globally installed (recommended for CI). JS repos: add `dorfl` to root `package.json` devDependencies (the CI shim prefers the project-local copy). Non-JS repos: a vendored `./bin/dorfl` or an `npx dorfl@<version>` / `mise` / `asdf` shim. Note your pin + how to bump it here. -->
