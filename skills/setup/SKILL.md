...
**Nudge to PIN the dorfl version (reproducibility — language-agnostic; fold into the A4 plan, do NOT add a separate question round).** Agents are taught to invoke bare **`dorfl`**, so the workflow stays project-independent — but bare `dorfl` runs whatever version is globally installed, which DRIFTS (a laptop on one version, CI floating to latest, a repo reasoned-about under a third). A repo can PIN the dorfl version it runs with, so builds/advances/intakes are reproducible. ASK once, generically: **"Do you want to PIN the dorfl version this repo runs with, so `dorfl` is reproducible instead of floating with whatever is globally installed? (recommended for CI.)"** If yes, declare the pin via the **`dorflCmd`** field in `dorfl.json` — the exact dorfl COMMAND bare `dorfl` self-forwards to — and note it under `## Conventions` in CONTEXT.md so it is discoverable. `dorflCmd` is honoured verbatim (dorfl does NOT resolve/download a version), so name whatever the repo's environment resolves: for a **JS repo**, add `dorfl` to the ROOT `package.json` `devDependencies` at the current version and set `"dorflCmd": "node_modules/.bin/dorfl"` (the `install-ci` CI shim also PREFERS the project-local bin, so CI stays aligned); for a **non-JS repo**, set `dorflCmd` to a vendored `"./bin/dorfl"`, an `"npx dorfl@<version>"`, or a `"mise exec dorfl@<version> --"` / `asdf` shim. Do NOT invent a JS dependency in a non-JS repo (the A3 language-agnostic rule). The full model, per-ecosystem examples, the announce + `--no-forward`/`DORFL_NO_FORWARD=1` opt-outs, and the upgrade ritual are documented in `docs/dorfl-cmd/README.md` — point the user there. If they skip, leave it — the global `dorfl` is used (floating); note the trade-off in one line so it is a conscious choice.
...
<!-- Reproducibility: consider PINNING the dorfl version this repo runs with via `dorflCmd` in dorfl.json, so bare `dorfl` self-forwards to a fixed version instead of floating with whatever is globally installed (recommended for CI). JS repos: add `dorfl` to root `package.json` devDependencies + `"dorflCmd": "node_modules/.bin/dorfl"`. Non-JS repos: `"dorflCmd"` = a vendored `"./bin/dorfl"`, `"npx dorfl@<version>"`, or a `"mise exec dorfl@<version> --"` / `asdf` shim. Note your pin + how to bump it here (bump `dorflCmd` -> `dorfl sync` -> re-run `install-ci` only if templates changed). See docs/dorfl-cmd/README.md. -->
