...
Do the moves with `git mv` so each rename is staged as a rename; sort the old `work/dropped/` contents by what each item IS (task vs prd vs observation) into the right destination. Present this mapping and STOP for confirmation (it touches the repo's live ledger); never auto-run it.
