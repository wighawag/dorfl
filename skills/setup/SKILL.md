...
- **Detect Phase B material.** While inventorying, note any **convertible material**: a `tasks/` folder, a `TODO.md`, an issue-tracker export, `docs/` design notes / RFCs, plan files, and **substantial source code** (code that embodies decisions). This detection decides whether Phase B runs (see A4 / Phase B). A repo with only a README that says "clean slate" (or nothing) → Phase B is empty; finish after Phase A.
...
- **Recommend removing fully-migrated SOURCE files — with confirmation, NEVER silently.** Once a source (`TODO.md`, a `tasks/` folder, a plan doc, a design note) is _fully_ captured into `work/` items, leaving the original creates **two sources of truth that drift**. In the report, LIST each fully-migrated source and propose deleting it ("`TODO.md` is now captured in `work/ideas/` + the PRD — delete it?"). Only delete on explicit user confirmation; never `rm` on your own initiative; never propose deleting a source you only _partially_ converted (say what's left). If the user declines, leave them and note they are superseded.
