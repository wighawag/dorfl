...
The **slicing discipline** the autonomous runner invokes by name on a brief in `work/briefs/ready/` to decompose it into independently-grabbable, file-based work tasks (tracer-bullet vertical slices). The runner spawns a fresh-context agent and tells it to "run the slicing protocol"; that agent reads THIS doc and applies its standard.

This is the runner-invoked counterpart to `CLAIM-PROTOCOL.md`, `REVIEW-PROTOCOL.md`, and `SURFACE-PROTOCOL.md`: the protocol contains how work is AUTHORED (`WORK-CONTRACT.md`, the templates), CLAIMED and BUILT (`CLAIM-PROTOCOL.md`, the Gate-1 `verify` floor), JUDGED BEFORE LANDING (`REVIEW-PROTOCOL.md`), has its OPEN QUESTIONS SURFACED (`SURFACE-PROTOCOL.md`), and — between brief and buildable task — SLICED (this doc). One contract, in-band in every set-up repo, never host-specific. (The human-facing pointer is `skills/to-task/SKILL.md`; the standard lives here.)
...
When the runner spawns you on the agent slicing path, you EDIT files only — write the slice files under the STAGING folder, trim the brief — and the RUNNER owns every git-state transition (it commits the produced slices, releases the slicing lock, and moves the brief into `work/briefs/tasked/`). Do not stage, commit, push, or move any files yourself. The runner integrates the slicing transition through the shared band (`--propose` PR / `--merge` main) honouring the caller's flags.

## The emitted slice shape (mirrors `work/protocol/task-template.md`)
...
- **`## What to build`** — a concise description of the vertical slice — the end-to-end behaviour (a thin path through every layer: schema → logic → API/UI → tests), NOT a layer-by-layer implementation plan. Avoid specific file paths / code snippets (they go stale).
...
The slicer ALWAYS writes emitted slice files to `work/tasks/backlog/` (the STAGING folder). The pool `work/tasks/todo/` is the agent-eligible pool the runner owns the promotion into; a write outside the staging folder is dropped by the runner-deterministic placement resolver. The agent never self-places into the pool.
