...
3. **Set the two autonomy axes (the prd now CARRIES the gate).** Because a prd may be AUTO-tasked by an agent with no human in the loop, decide and record both:
   ...
   - If this prd's tasks will depend on another prd's tasks, set `prdAfter: [other-prd]` so it is tasked in the right order. With auto-tasking the ordering must be pre-recorded here OR honestly flagged, since there may be no human at tasking time to supply it.
