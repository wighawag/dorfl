...
/**
 * `dorfl work-on` — the HUMAN command to claim a task and create an
 ...
 * `~/.dorfl/` (that is the agents' area; `work-on` is human-only).
 *
 ...
 *
 *   work-on() { cd "$(dorfl work-on "$@" --print-dir)"; }
 */
...
async function runWorkOn(
	...
	if (!slug) {
		throw new WorkOnUsageError(
			'missing <slug>. usage: dorfl work-on <slug> [--remote <r>]',
		);
	}
