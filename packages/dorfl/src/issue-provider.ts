 ...
 * spec `issue-intake` US #10): a TRANSIENT concurrency mutex — added on start (the
 * winner only), removed on finish — that serialises two concurrent `intake` runs on
 * the SAME issue. It is namespaced under the brand (`dorfl:processing`) so it
...
/**
 * The **issue seam** (spec `issue-intake`, task `intake-tracer-slice-outcome`):
 ...
 * task/spec path; only the BOUNCE close is intake's.
 *
...
export interface LabelResult {
	 ...
	 * `gh` stderr (e.g. `'dorfl:processing' not found`), NOT a hard-coded
...
export interface IssueProvider {
	 ...
	 * later, by CI's task/spec close-job. The RUNNER calls this; the agent stays
	...
	private mutateLabel(
		...
		if (result.status !== 0) {
			...
			return {
				...
				applied: false,
				reason,
				// Surface the REAL `gh` stderr (e.g. `'dorfl:processing' not found`),
	...
	private createLabel(
		...
		const result = this.runGh(
			[
				...
				label,
				'--description',
				'dorfl intake processing lock (transient concurrency mutex)',
			],
			cwd,
			env,
		);
