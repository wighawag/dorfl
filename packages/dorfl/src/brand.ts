 ...
 * bearing strings).
 *
 * dorfl's name leaks into a handful of strings that are a CONTRACT with
 ...
 *   - the `dorfl` binary / package name where referenced FROM CODE.
 *
 ...
 * cosmetic doc/prose mentions of "dorfl" (ADRs, prds, CONTEXT.md, tasks,
...
export interface Brand {
	/** The base identity string (paramCase): `dorfl`. */
	...
	/** The config-dir name (under `~/.config/`): `paramCase(base)` ⇒ `dorfl`. */
	readonly configDirName: string;
	/** The binary / package name (paramCase): `dorfl`. */
	readonly bin: string;
}
