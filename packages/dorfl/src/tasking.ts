...
function buildTaskingSpec(slug: string, _spec: string | undefined): string {
	...
	return [
		...
		`COMPLETENESS CHECK across the spec's user stories FIRST. Every story becomes a`,
		`task now, or NONE does — there is no "partially tasked" state. If ANY story is`,
		`gated / deferred / unanswered (needs an unresolved question, or is "direction,`,
		`not commitment"), the spec is MIS-SCOPED: do NOT task the confident subset.`,
		`Stop and route the spec to needs-attention (it must be SPLIT into a`,
		`fully-taskable spec plus a separate spec for the gated remainder) rather than`,
		`emitting tasks for only part of it.`,
		``,
