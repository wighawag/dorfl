...
let scratch: Scratch;
beforeEach(() => {
	scratch = makeScratch('dorfl-item-lock-');
});
afterEach(() => {
	scratch.cleanup();
});
...
describe('item-lock — identity seam (reuses resolveSidecarIdentity)', () => {
	it('derives the type-encoded <entry> from the namespaced identity', () => {
		...
		// `spec:` gets its OWN `spec-<slug>` entry (the parent-spec type token).
		...
		// HARD CUTOVER: `prd:` is no longer a namespace prefix — `prd:autotask`
		// resolves as a bare literal TASK slug (`task-prd:autotask`), never `prd-…`.
		expect(lockEntryFor('prd:autotask')).toBe('task-prd:autotask');
	});

	it('a task, a SPEC, and an observation sharing a slug get DISTINCT refs', () => {
		const slug = 'shared';
		// `spec:` must NOT collide with the same-slug `task:` build lock (the
		// isolation invariant).
		const refs = new Set([
...
describe('item-lock — happy path', () => {
	...
	it('does NOT touch main (the item file stays in backlog/, no main move)', async () => {
		...
		const stillBacklog = run(
			'git',
			['cat-file', '-e', 'arbiter/main:work/tasks/ready/alpha.md'],
			repo,
			{env: gitEnv()},
		);
