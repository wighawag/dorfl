 ...
 *       runner-owned function the tasking path does not call.
 */
...
/** An agent that writes one staged task file under `work/tasks/backlog/`. */
function stagingAgent(file = 'child'): TaskDorfl {
	return ({cwd}) => {
 ...
 */
function selfPlacingAgent(legit: string, hijack: string): TaskDorfl {
	return ({cwd}) => {
...
describe('STEP A — work/tasks/ready/ STILL means the agent-eligible pool (readers unchanged)', () => {
	it('a STAGED slug is NOT claimable (the claim CAS reads work/tasks/ready/ only)', async () => {
		...
		await performTask({
			...
			autoTask: true,
			integration: 'merge',
			dorfl: stagingAgent('staged-only'),
			env: gitEnv(),
		});
...
describe('STEP A — the agent cannot self-place into the pool (pool-placement fence)', () => {
	it('an agent that writes work/tasks/ready/<hijack>.md during tasking has its write scrubbed; only work/tasks/backlog/<legit>.md lands', async () => {
		...
		const result = await performTask({
			...
			autoTask: true,
			integration: 'merge',
			dorfl: selfPlacingAgent('legit', 'hijack'),
			env: gitEnv(),
		});
		expect(result.outcome).toBe('tasked');
	...
	it('a pool-edit attempt (the agent overwrites a PRE-EXISTING pool task during tasking) is also reverted (HEAD content preserved)', async () => {
		...
		const tamperingAgent: TaskDorfl = ({cwd}) => {
			const staged = join(cwd, 'work', 'tasks', 'backlog');
			mkdirSync(staged, {recursive: true});
			writeFileSync(
 ...
 * exit (a usage error, e.g. a bad `--tasks-land-in`) leaves `code` undefined and
