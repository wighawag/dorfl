 ...
 * STUBBED agent (injected `dorfl`, never a real harness). The end-to-end
...
beforeEach(() => {
	scratch = makeScratch('dorfl-do-isolated-');
...
describe('do --isolated + --remote — REDUNDANT: remote wins (isolation implied)', () => {
	it('with --remote present, --isolated is a no-op: the cwd arbiter is NOT consulted (no “isolated against what?” error even with no cwd arbiter)', async () => {
		...
		// materialises in scratch, never the real ~/.dorfl) and NO agentCmd
...
describe('do --isolated — -n/auto-pick now SELECTS over the mirror-side pool (refusal removed)', () => {
	...
	// ~/.dorfl). With `autoBuild` off (the default) the mirror scan selects
...
describe('do --isolated -n — SEQUENTIAL-REFETCH FRESHNESS drain (item N rebases onto item N-1’s merge)', () => {
	...
	it('integrates item 1 THEN item 2, with item 2’s worktree branched off a main containing item 1’s merge', async () => {
		...
		const recordingAgent: DoDorfl = ({cwd, slug}) => {
