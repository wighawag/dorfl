...
describe('the close-job workflow satisfies every structural invariant', () => {
	...
	it('invokes `dorfl close-merged-issues` (consumes the unchanged resolution + query + close)', () => {
		const text = generateCloseJobWorkflow(config);
		expect(/dorfl close-merged-issues\b/.test(text)).toBe(true);
...
describe('validateCloseJobWorkflow flags a workflow missing each invariant', () => {
	...
	it('flags a missing `dorfl close-merged-issues` invocation', () => {
		expectFlagged(
			base.replace(/run: dorfl close-merged-issues/, 'run: echo nothing'),
	...
	it('flags a direct `gh issue close` (must go through the provider seam)', () => {
		expectFlagged(
			base.replace(/run: dorfl close-merged-issues/, 'run: gh issue close 42'),
			'no-direct-gh-issue-close',
		);
	});
	...
	it('flags an --isolated flag (CI runs in-place)', () => {
		expectFlagged(
			base.replace(
				/run: dorfl close-merged-issues/,
				'run: dorfl close-merged-issues --isolated',
			),
			'no-isolated-flag',
		);
	});
