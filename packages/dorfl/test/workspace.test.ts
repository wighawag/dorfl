...
let scratch: Scratch;
beforeEach(() => {
	scratch = makeScratch('dorfl-workspace-');
});
afterEach(() => {
	scratch.cleanup();
});
...
describe('encodeWorkId', () => {
	it('is flat: host__org__name__slug (dot→dash per segment), reusing the repo-key encoding', () => {
		expect(encodeWorkId('git@github.com:wighawag/dorfl.git', 'my-task')).toBe(
			'github-com__wighawag__dorfl__my-task',
		);
	});

	it('encodes an https URL the same way', () => {
		expect(encodeWorkId('https://github.com/wighawag/dorfl.git', 'feat')).toBe(
...
describe('jobWorktreePath', () => {
	it('locates <workspacesDir>/work/<work-id>/ (flat, outside the hub)', () => {
		...
		expect(p).toBe('/home/me/.dorfl/work/github-com__o__r__feat');
	});
