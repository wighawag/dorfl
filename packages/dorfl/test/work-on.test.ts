...
beforeEach(() => {
	scratch = makeScratch('dorfl-work-on-');
});
afterEach(() => {
	scratch.cleanup();
});

/** A configured human worktree root inside the scratch (never ~/.dorfl). */
...
describe('work-on — in-repo form (work-on <slug>)', () => {
	it('claims and creates a worktree on work/<slug> off the fresh arbiter main', async () => {
		...
		// The worktree lives UNDER the configured human root, never ~/.dorfl.
		expect(result.dir!.startsWith(humanRoot())).toBe(true);
		expect(result.dir).not.toMatch(/\.dorfl/);
	...
	it('a two-claimer race: the loser creates no worktree, the winner gets one', async () => {
		...
		const [ra, rb] = await Promise.all([
			performWorkOn({
				...
				cwd: a,
				arbiter: 'arbiter',
				workspacesDir: join(scratch.root, '.dorfl-a'),
			...
			performWorkOn({
				...
				cwd: b,
				arbiter: 'arbiter',
				workspacesDir: join(scratch.root, '.dorfl-b'),
...
describe('work-on — humanWorktreesDir (first-use prompt + guards)', () => {
	...
	it('refuses a humanWorktreesDir under ~/.dorfl (agents-only area)', async () => {
		const {repo} = seedRepoWithArbiter(scratch.root, ['alpha']);
		const result = await performWorkOn({
			slug: 'alpha',
			cwd: repo,
		...
		expect(result.exitCode).toBe(1);
		expect(result.outcome).toBe('usage-error');
		expect(result.message).toMatch(/\.dorfl/);
	});
	...
	it('suggestHumanWorktreesDir avoids the ~/dev code-dir prefix (tab-completion)', () => {
		...
		expect(suggestion).not.toMatch(/\.dorfl/);
	});
});
