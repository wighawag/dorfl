...
let scratch: Scratch;
beforeEach(() => {
	scratch = makeScratch('dorfl-fwg-test-');
});
afterEach(() => {
	scratch.cleanup();
});
...
function gateSandboxCount(): number {
	return readdirSync(tmpdir()).filter((d) => d.startsWith('dorfl-fresh-gate-'))
		.length;
}
...
describe('fresh-worktree gate — reaped after the gate (pass OR fail), never leaks', () => {
	it('a GREEN gate reaps the throwaway worktree (no dorfl-fresh-gate-* dir lingers)', async () => {
		...
		expect(worktrees).not.toMatch(/dorfl-fresh-gate-/);
	});
...
describe('fresh-worktree gate ON + review ON — verify-THEN-review, both on the REBASED tip (MAINTAINER DECISION 2)', () => {
	it('the review runs AFTER the rebased-tip verify, on the SAME (rebased-tip) tree', async () => {
		...
		expect(reviewCwd).toMatch(/dorfl-fresh-gate-/);
