...
describe('freshWorktreeGate — env coercion (typed, loud)', () => {
	it('coerces DORFL_FRESH_WORKTREE_GATE as a boolean', () => {
		expect(
			envOverrides({DORFL_FRESH_WORKTREE_GATE: 'false'}).freshWorktreeGate,
		).toBe(false);
		expect(
			envOverrides({DORFL_FRESH_WORKTREE_GATE: 'true'}).freshWorktreeGate,
		).toBe(true);
	});

	it('names the env var by the SCREAMING_SNAKE convention', () => {
		expect(envVarName('freshWorktreeGate')).toBe('DORFL_FRESH_WORKTREE_GATE');
	});

	it('fails LOUDLY on a non-boolean value', () => {
		expect(() => envOverrides({DORFL_FRESH_WORKTREE_GATE: 'maybe'})).toThrow(
			/DORFL_FRESH_WORKTREE_GATE/,
		);
	});
});
	...
	it('flag > env > per-repo > global', () => {
		...
		try {
			...
			const envWins = resolveRepoConfig({
				...
				env: {DORFL_FRESH_WORKTREE_GATE: 'false'},
			});
