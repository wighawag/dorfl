...
let scratch: Scratch;
beforeEach(() => {
	scratch = makeScratch('dorfl-arbiter-');
});
afterEach(() => {
	scratch.cleanup();
});
...
/**
 * Seed a plain working repo (no arbiter) with a `work/tasks/ready/<slug>.md` for
...
describe('arbiterPath', () => {
	it('locates <arbitersDir>/<host>/<org>/<name>.git, hierarchical, dot→dash per segment', () => {
		expect(
			arbiterPath('/home/me/git', 'git@github.com:wighawag/dorfl.git'),
		).toBe('/home/me/git/github-com/wighawag/dorfl.git');
	});
	...
	it('is NEVER under ~/.dorfl (precious data, ADR §7)', () => {
		const p = arbiterPath('/home/me/git', 'https://github.com/o/r.git');
		expect(p).not.toMatch(/\.dorfl/);
...
describe('arbiter init', () => {
	it('creates a bare arbiter at the resolved ~/git path and wires the repo remote', () => {
		...
		// Resolved under arbitersDir, hierarchical, *.git, NEVER ~/.dorfl.
		const expectedPath = arbiterPath(arbitersDir, `file://${repo}`);
		expect(result.path).toBe(expectedPath);
		expect(result.path).not.toMatch(/\.dorfl/);
	...
	it('refuses to clone a non-bare repo with main checked out (the unsafe case)', () => {
		...
		mkdirSync(join(nonBare, 'work', 'tasks', 'ready'), {recursive: true});
...
describe('claim against a provisioned arbiter (end-to-end)', () => {
	it('a claim succeeds end-to-end after arbiter init', async () => {
		...
		const inBacklog = git(
			['cat-file', '-e', 'arbiter/main:work/tasks/ready/feat.md'],
			clone,
			{env: gitEnv()},
		);
