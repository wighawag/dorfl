...
describe('config-override paths', () => {
	...
	it('the real ~/.config/dorfl/ is NEVER read by the loader unless we name it', () => {
		...
		// touch the real `~`.)
		const scratch = mkdtempSync(join(tmpdir(), 'dorfl-override-'));
		try {
...
describe('loadConfigOverride', () => {
	...
	let path: string;
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), 'dorfl-override-'));
	...
	it('reads a valid map verbatim (`*` + a hub-key bucket)', () => {
		...
		expect(map['github-com/wighawag/dorfl']).toEqual({
			integration: 'merge',
		});
	});
...
describe('resolveRepoConfigFromLoaded \u2014 override layering', () => {
	...
	const HUB = encodeRepoKey(URL); // `github-com/wighawag/dorfl`
	...
	it('FLAG > ENV > override:hub > override:* > committed > global > default', () => {
		...
		const env = {DORFL_INTEGRATION: 'propose'} as NodeJS.ProcessEnv;
		...
		const envWins = resolveRepoConfigFromLoaded(
			{path: 'x', config: {integration: 'propose'}, rejected: []},
			{
				...
				arbiterUrl: URL,
				env: {DORFL_INTEGRATION: 'propose'} as NodeJS.ProcessEnv,
			},
		);
...
describe('resolveRepoConfig \u2014 working-tree path resolves the arbiter URL itself', () => {
	let repo: string;
	beforeEach(() => {
		repo = mkdtempSync(join(tmpdir(), 'dorfl-override-rt-'));
...
describe('resolveRepoConfigFromMirror \u2014 mirror path resolves origin itself', () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), 'dorfl-override-mirror-'));
