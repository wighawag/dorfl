...
describe('env-config — the keys are coerced (typed, loud)', () => {
	it('coerces review as a boolean and reviewMaxRounds as a number', () => {
		const env = {
			DORFL_REVIEW: 'true',
			DORFL_REVIEW_MODEL: 'env/review',
			DORFL_REVIEW_MAX_ROUNDS: '3',
		};
	...
	it('DORFL_AUTO_MERGE is inert — not a recognised env var', () => {
		// autoMerge is gone, so its env var no longer resolves to any config key.
		const o = envOverrides({DORFL_AUTO_MERGE: 'true'});
		expect('autoMerge' in o).toBe(false);
	});

	it('names the env vars by the SCREAMING_SNAKE convention', () => {
		expect(envVarName('review')).toBe('DORFL_REVIEW');
		expect(envVarName('reviewModel')).toBe('DORFL_REVIEW_MODEL');
		expect(envVarName('reviewMaxRounds')).toBe('DORFL_REVIEW_MAX_ROUNDS');
	});

	it('fails LOUDLY on a non-boolean review', () => {
		expect(() => envOverrides({DORFL_REVIEW: 'yes'})).toThrow(/DORFL_REVIEW/);
	});
});
