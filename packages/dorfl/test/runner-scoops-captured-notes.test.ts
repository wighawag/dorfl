...
beforeEach(() => {
	scratch = makeScratch('dorfl-scoop-notes-');
...
/** Seed a `work/specs/ready/<slug>.md` (committed onto the arbiter) for the tasking path. */
...
function taskingAgentWithNote(note: string | undefined): TaskDorfl {
	return ({cwd}) => {
...
describe('the TASK path (do prd:) scoops + reports agent-authored captured notes', () => {
	...
	it('a tasking run that writes NO captured note ⇒ no scoop report', async () => {
		...
		const result = await performTask({
			...
			autoTask: true,
			integration: 'merge',
			dorfl: taskingAgentWithNote(undefined),
			env: gitEnv(),
			note: sink.note,
		});
