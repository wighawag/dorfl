 ...
 *     `lost`), never `--force`;
 *   - it covers all four terminals (task done/cancelled, spec tasked/dropped);
...
let scratch: Scratch;
beforeEach(() => {
	scratch = makeScratch('dorfl-gc-reap-');
});
afterEach(() => {
	scratch.cleanup();
});
			...
			entries: [
				{
					...
					ref: 'refs/dorfl/lock/task-raced',
					outcome: 'lost',
					message:
