 ...
 *     uses `issue:` XOR `brief:`; a lone task closes its own `issue:` directly, a
 *     fanned task reaches the number via `task.brief: → work/briefs/<brief>.md brief
 *     issue:`, and `brief:` WINS on a (hand-edited) conflict;
 *   - the QUERY — {@link isBriefComplete} (`brief-complete.ts`, task
 *     `prd-complete-query`, done): a brief is COMPLETE iff ≥1 `brief:<slug>` task AND
 ...
 *
 *   - a **lone task** (`issue:`, no `brief:`) that resides in `work/done/` — its PR
 *     merged, so its own issue closes (reason `completed`);
 *   - a **brief** (`issue:`) — closes ONLY when {@link isBriefComplete} says ALL its
 *     `brief:<slug>` tasks are in `work/done/` (reason `completed`).
 *
 * A brief whose query is NOT yet complete is left OPEN (the final fanned task's
...
export type CloseDecision =
	...
	| 'not-complete' // a brief whose query says it is not yet complete → left open
...
/**
 * Read a brief's `issue:` number by slug, scanning the brief folders. Returns
 * `undefined` when no brief with that slug carries an `issue:` (a brief with no source
 * issue, or a typo'd `brief:` hop — degrades to "no issue to close", never crashes).
 */
 ...
 *
 *   - every brief carrying `issue:` (in `work/briefs/ready/` or `work/briefs/tasked/`) is a
 *     brief-kind candidate keyed on its own `brief:` query;
 *   - every LONE task (`issue:` and NO `brief:`) is an `issue`-kind candidate.
 *
 * A fanned task carries `brief:` (NOT its own `issue:`), so it reaches the number
 * through its brief's candidate, never as its own — the issue number lives ONLY on
 * the brief. Deduplicated by issue number: a brief enumerated once even though many
 * tasks point at it. Brief candidates are listed before lone-task candidates, each
...
function resolveCandidates(repoPath: string): {
	...
	// Brief candidates: a brief's `issue:` closes when ITS query is complete.
	for (const folder of BRIEF_FOLDERS) {
		for (const file of listMarkdown(repoPath, folder)) {
			const fm = parseFrontmatter(
				readFileSync(join(repoPath, workItemRel(folder, file)), 'utf8'),
			...
			// A brief's own closing link is its `issue:` (a brief has no `brief:`); on the
			// hand-edit conflict `brief:` wins via resolveClosingIssue, so only a true
	...
	// Lone-task candidates: a task with `issue:` and NO `brief:` closes its OWN
	...
	for (const folder of TASK_FOLDERS) {
		for (const file of listMarkdown(repoPath, folder)) {
			...
			const slug = fm.slug ?? basename(file, '.md');
			const closing = resolveClosingIssue(fm);
			// `resolveClosingIssue` returns `via: 'issue'` ONLY when there is no `brief:`
			// (brief wins on conflict), so a fanned task never lands here — it reaches
			// its issue through the brief candidate above.
			if (closing?.via === 'issue' && !seen.has(closing.issue)) {
				seen.add(closing.issue);
 ...
 * apply the per-kind closure condition (a landed lone task; a brief whose
