 ...
 * mirror scan only enumerates eligible slices (gated on `allowAgents`) + sliceable
...
export interface PerformDoRemoteAutoOptions extends SharedRemoteOptions {
	/**
	 * The resolved (remote) repo config — provides `allowAgents`/`autoSlice` (the
...
export async function performDoRemoteAuto(
	...
	// enumerates (eligible slices gated on `allowAgents`, sliceable PRDs gated on
