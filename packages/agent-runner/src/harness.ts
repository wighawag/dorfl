...
export interface LaunchInput {
	...
	model?: string;
	/**
	 * The absolute pi session FILE path (`--session <path>`, ending `.jsonl`) the
	 * caller generated for THIS launch (see `session-path.ts`). The pi adapter
	 * passes it verbatim and records it; the `do --watch` tailer tails the SAME
	 * path. Generated by the CALLER (not the adapter) because the watcher needs it
	 * BEFORE the launch starts. Other adapters (null/shell) ignore it.
	 */
	session?: string;
