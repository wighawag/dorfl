...
jobs:
  ...
  # Build the DYNAMIC matrix from the mirror-side eligible-pool scan. \`scan --json\`
  # reports the eligible items; \`jq\` shapes them into a deduplicated GitHub Actions
  # matrix list of explicit \`slice:<slug>\` ids (CI MUST use explicit prefixes).
  # Each becomes one matrix leg → one independent \`advance\` → one PR. Skipped in
  # merge mode.
  enumerate:
