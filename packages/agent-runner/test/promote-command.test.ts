...
/** Stage a PRD file in `work/pre-prd/` on the arbiter. */
...
describe('listPromotable — reads the arbiter staging folders', () => {
	it('lists slices in pre-backlog/ then PRDs in pre-prd/, sorted, from the arbiter (not the local tree)', async () => {
		...
		expect(result.items).toEqual([
			{namespace: 'slice', slug: 'alpha-slice'},
			{namespace: 'slice', slug: 'beta-slice'},
...
describe('promote [item] — no argument LISTS what is staged', () => {
	it('lists staged slices + PRDs as `namespace:slug` lines', async () => {
		...
		expect(out).toMatch(/slice:my-slice/);
...
describe('promote <item> — admits a staged item into its pool', () => {
	...
	it('promote prd:<slug> moves pre-prd/ -> prd/ on the arbiter (auto-sliceable)', async () => {
		...
		expect(onArbiterMain(repo, 'work/pre-prd/vision.md')).toBe(true);
		const {out} = await runPromote(repo, ['prd:vision']);
		expect(out).toMatch(/Promoted prd 'vision' into the pool/);
		expect(onArbiterMain(repo, 'work/prd/vision.md')).toBe(true);
		expect(onArbiterMain(repo, 'work/pre-prd/vision.md')).toBe(false);
	});
});
...
describe('promote <item> — error cases fail loud', () => {
	it('a not-staged slug exits non-zero with an honest reason', async () => {
		const {repo} = seedRepoWithArbiter(scratch.root, []);
		const {err, code} = await runPromote(repo, ['slice:ghost']);
		expect(code).toBe(1);
