...
import {runLoop, runOnce, type AgentRunner} from '../src/run.js';
 ...
 * agent (injected `agentRunner` edits files directly). Real git, writes main,
...
beforeEach(() => {
	scratch = makeScratch('agent-runner-run-uses-advance-tick-');
...
describe('advanceRegistrySetRunTick — no regression in needs-attention routing', () => {
	it(
		...
		async () => {
			...
			const failingAgent: DoAgentRunner = () => ({
				ok: false,
			...
			const summary = await runLoop({
				config: config(),
				tick: advanceRegistrySetRunTick(deps({}, {agentRunner: failingAgent})),
				maxIterations: 1,
				sleep: async () => {},
				env: gitEnv(),
			});
