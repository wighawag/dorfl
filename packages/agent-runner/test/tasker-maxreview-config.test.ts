...
describe('taskerLoopMax — env coercion (typed, loud)', () => {
	it('coerces AGENT_RUNNER_TASKER_LOOP_MAX as a number', () => {
		expect(
			envOverrides({AGENT_RUNNER_TASKER_LOOP_MAX: '5'}).taskerLoopMax,
		).toBe(5);
	});

	it('names the env var by the SCREAMING_SNAKE convention', () => {
		expect(envVarName('taskerLoopMax')).toBe('AGENT_RUNNER_TASKER_LOOP_MAX');
	});

	it('fails LOUDLY on a non-numeric value', () => {
		expect(() => envOverrides({AGENT_RUNNER_TASKER_LOOP_MAX: 'abc'})).toThrow(
			/AGENT_RUNNER_TASKER_LOOP_MAX/,
		);
	});
});
	...
	it('flag > env > per-repo > global', () => {
		...
		try {
			...
			const envWins = resolveRepoConfig({
				...
				env: {AGENT_RUNNER_TASKER_LOOP_MAX: '6'},
			});
