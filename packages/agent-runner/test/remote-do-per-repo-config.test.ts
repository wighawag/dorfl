 ...
 * carries a `.agent-runner.json` (with BOTH allowed + rejected keys), a temp
...
beforeEach(() => {
	scratch = makeScratch('agent-runner-remote-repo-config-');
...
describe('do --remote — reads the per-repo .agent-runner.json from the arbiter main', () => {
	...
	it('an AGENT_RUNNER_* env var OVERRIDES the per-repo file (env > per-repo)', () => {
		const {arbiter} = seedRepoWithArbiter(scratch.root, ['alpha'], {
			repoConfig: {harness: 'pi', model: 'repo-model'},
		});
		const {config} = resolve({
			arbiter,
			env: {AGENT_RUNNER_MODEL: 'env-model'},
		});
	...
	it('a config-less repo resolves to global+default (byte-identical to today)', () => {
		// No `repoConfig` → no `.agent-runner.json` committed on main.
		const {arbiter} = seedRepoWithArbiter(scratch.root, ['alpha']);
...
describe('readRepoConfigFromMirrorMain — sources the committed bytes from <arbiter>/main', () => {
	...
	it('returns undefined when the repo has NO .agent-runner.json on main', () => {
		const {arbiter} = seedRepoWithArbiter(scratch.root, ['alpha']);
		const env = gitEnv();
		const mirror = ensureMirror({
			url: remoteUrl(arbiter),
	...
	it('the sourced filename matches the brand REPO_CONFIG_FILENAME', () => {
		...
		// test fixture writes that same filename (`.agent-runner.json`).
