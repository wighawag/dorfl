...
describe('intake marker \u2014 stamp + parse the hidden HTML comment', () => {
	...
	it('uses the brand.base namespace (today agent-runner:intake), so a rebrand updates it', () => {
		const out = stampIntakeMarker('x', {kind: 'bounced', seen: ['503']});
		expect(out).toContain(`<!-- agent-runner:intake `);
		expect(NS).toBe('agent-runner:intake');
	});
