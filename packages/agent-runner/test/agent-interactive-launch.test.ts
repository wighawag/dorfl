...
beforeEach(() => {
	scratch = makeScratch('agent-runner-interactive-');
...
describe('start --agent — interactive launch after onboarding', () => {
	it('invokes the interactive launcher with the checkout cwd after a winning claim', async () => {
		...
		// It is NOT a tracked job (decision #3): no .agent-runner-job.json written
		// in the checkout.
		expect(existsSync(join(repo, '.agent-runner-job.json'))).toBe(false);
	});
...
describe('work-on --agent — interactive launch in the new worktree', () => {
	it('invokes the interactive launcher with the worktree dir after creation', async () => {
		...
		expect(existsSync(join(result.dir!, '.agent-runner-job.json'))).toBe(false);
	});
	...
	it('does NOT launch when the claim is lost (no worktree)', async () => {
		...
		await performWorkOn({
			...
			cwd: other,
			arbiter: 'arbiter',
			workspacesDir: join(scratch.root, '.agent-runner-other'),
	...
	it('the real ~/.agent-runner + ~/.pi/agent/sessions are untouched (isolation)', async () => {
