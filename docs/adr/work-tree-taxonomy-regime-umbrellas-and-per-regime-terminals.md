---
title: The work/ tree is grouped into regime umbrellas (notes/tasks/briefs) with two DISTINCT lifecycles and per-regime won't-proceed terminals; questions/ stays top-level
status: proposed
created: 2026-06-19
supersedes:
superseded_by:
...
# ADR: regime umbrellas for the work/ tree (notes / tasks / briefs), two deliberately-different lifecycles, per-regime terminals, and a top-level questions/
...
2. **A task and a brief are fundamentally DIFFERENT kinds of thing, so they get
   different lifecycles, not one forced shape.** A brief is something you DECOMPOSE;
   ...
   board. Forcing the brief regime into the same board vocabulary would mislead a
...
- **`draft` for the brief staging slot.** Rejected per Why #3: it implies an
