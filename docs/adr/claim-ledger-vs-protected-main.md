...
- **Ledger ↔ code-merge reconciliation/cleanup** — the intermediate signal and "done on `main`" would live on different refs/timelines; how are they reconciled if a PR merges but the intermediate wasn't cleaned up (or vice-versa)? (Same class as the issue→spec `Fixes #N` loop-closure question.)
