...
> spec (`work/specs/tasked/advance-loop.md`, "The sidecar FORMAT (RESOLVED here)") and the design
...
- The spec-resolved "tooling-owned, byte-exact round-trip" property is deliberately RELAXED to
...
- `serialiseSidecar`/`parseSidecar` and `sidecar.test.ts` are rewritten; the surface-questions skill
  ...
  spec's (`work/specs/tasked/advance-loop.md`) "sidecar FORMAT (RESOLVED here)" section is updated
  to point here.
...
- A future reader seeing Markdown + HTML comments instead of the spec's documented YAML-ish format
