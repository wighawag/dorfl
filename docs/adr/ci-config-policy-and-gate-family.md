...
`runner-in-ci` (`work/specs/tasked/runner-in-ci.md`) makes CI run the autonomous rungs
...
`advance` is a strict superset of `do` (build task / task spec), adding the
...
| task an undeclared spec | `autoTask` (bool) | rung does not run |
...
- **`surfaceBlockers` (bool, default `off`)** governs DECLARED work that is blocked:
  whether a task/spec carrying `needsAnswers: true` is rendered into an answerable
...
- **`selectionOrder` ranks the other four** (`build` / `task` / `surface` /
  ...
  ("tasks before specs" is just `build` before `task`), which is removed.
...
CI derives the integration mode per artifact from the downstream gate (a spec merged
...
non-default opt-in. See `work/specs/tasked/runner-in-ci.md` for the policy table; it is
...
- Cross-refs: `command-surface-and-journeys.md` (the gate family + the autonomous face), `work/specs/tasked/runner-in-ci.md` ("Config & gate model in CI"), `config.ts`/`repo-config.ts`/`env-config.ts` (the gate-family plumbing), `advance-loop-driver.ts` ("run == CI: swap the tick, keep the loop").
